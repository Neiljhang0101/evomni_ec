...
| 版本 | 日期 | 修改內容 | 修改人 |
|------|------|----------|--------|
| v1.2 | 2026/05/28 | 依議題 2026-05-20-integration-advanced-payment-list PM 決議：新增 §2.3 方案金流/物流/電子發票完整規格對照表（電商啟航/進階電商包各自標配與加費清單）；§8 移除「進階電商包自選 6 種金流待定」與「ezPay 加費條件待定」兩項待確認事項 | Una |
| v1.1 | 2026/05/28 | §8.5.1 移除 CREATE TABLE SQL；改以業務欄位需求表描述 payment_webhook_logs 欄位定義；工程師重點移除 Queue/5秒等實作細節，改以業務語意說明 | Una |
| v1.0 | 2026/04/27 | 初稿建立 | Una |

# Evomni — 金物流串接規格 產品需求文件 (PRD) v1.2
...
| 文件狀態 | **v1.2** — 方案金流/物流/電子發票完整規格定案（依 PM 決議 2026-05-28），詳見 §2.3 |
| 作者 | Una |
| 對應方案 | 電商啟航方案 ✅ / 進階電商包 ✅（差異見 §2.3 方案規格對照表）|
| 特別說明 | 方案金流/物流規格已於 v1.2 完整定案；電子發票：兩方案均內含綠界科技，另可加費串接 ezPay。|
...
| 金流 - 綠界 | 信用卡定期定額 | 兩方案共有 | 訂閱型商品支付 | 消費者 |
...
### 2.3 方案金流／物流／電子發票完整規格對照表

> **定案日期：2026/05/28（議題 2026-05-20-integration-advanced-payment-list 關閉）**

#### 電商啟航方案

| 類別 | 項目 | 說明 |
|------|------|------|
| **金流 — 自訂金流** | 後台自訂傳統付款方式 | 如 ATM 轉帳、匯款；消費者結帳後手動回報後五碼供人工對帳 |
| **金流 — 銀行金流** | 自選 1 種（14 家可選）| 永豐銀行(豐收款) / 兆豐銀行 / 國泰世華 / 玉山銀行 / 華南銀行 / 富邦銀行 / 彰化銀行 / 聯合信用卡中心NCCC / 第一銀行 / 合作金庫 / 聯邦銀行 / 元大銀行(僅支付寶) / 中國信託 / 台新銀行；其餘可加費串接 |
| **金流 — 第三方金流** | 方案內含 4 家 | 綠界科技（含定期定額扣款）/ 藍新科技 / LINE Pay / AFTEE 先享後付 |
| **金流 — 第三方金流** | 加費串接（9 家可選）| 紅陽科技 / 客樂得 / 歐付寶 / 萬事達 / smilepay 速買配 / Paypal / PchomePAY 支付連 / 網際威信 / 街口支付 |
| **物流 — 第三方物流** | 方案內含 2 家 | 綠界科技（含超商門市串接）/ 藍新科技（含超商門市串接）|
| **物流 — 第三方物流** | 加費串接（5 家可選）| 紅陽科技 / 統一數網 / 低溫超取_聖喆國際 / 倉儲物流_BOXFUL任意存 / 倉儲物流_ECFIT |
| **電子發票** | 方案內含 1 家 | 綠界科技電子發票 |
| **電子發票** | 加費串接 | ezPay（費用由業務/PM 另行規劃）|

#### 進階電商包

| 類別 | 項目 | 說明 |
|------|------|------|
| **金流 — 自訂金流** | 後台自訂傳統付款方式 | 同啟航方案 |
| **金流 — 銀行金流** | 自選 1 種（14 家可選）| 同啟航方案廠商清單；其餘可加費串接 |
| **金流 — 第三方金流** | **自選 6 種**（13 家可選）| 綠界科技（含定期定額扣款）/ 藍新科技 / 紅陽科技 / 客樂得 / 歐付寶 / 萬事達 / smilepay 速買配 / Paypal / AFTEE 先享後付 / LINE Pay / PchomePAY 支付連 / 網際威信 / 街口支付；其餘可加費串接 |
| **物流 — 第三方物流** | 方案內含 4 家 | 綠界科技 / 藍新科技 / 紅陽科技 / 統一數網（均含超商門市串接）|
| **物流 — 第三方物流** | 加費串接（3 家可選）| 低溫超取_聖喆國際 / 倉儲物流_BOXFUL任意存 / 倉儲物流_ECFIT |
| **電子發票** | 方案內含 1 家 | 綠界科技電子發票 |
| **電子發票** | 加費串接 | ezPay（費用由業務/PM 另行規劃）|
...
| Items | 商品明細陣列（ItemName, ItemCount, ItemUnit, ItemPrice, ItemAmount）| 訂單商品 |
| Vat | 課稅類別（1=應稅）| 固定值 1 |
...
冪等性 Key 使用 `vendor + merchant_trade_no + vendor_trade_no` 組合。

Webhook 紀錄表（`payment_webhook_logs`）欄位需求：

| 欄位 | 業務說明 |
|------|---------|
| 金流廠商 | 四種值：ecpay（綠界）/ newebpay（藍新）/ linepay / aftee |
| 本系統訂單號 | 對應 `merchant_trade_no` |
| 廠商端交易流水號 | 廠商回傳的交易識別碼 |
| 接收時間 | Webhook 抵達時間 |
| 原始 Payload | 廠商推送的完整內容，供除錯用 |
| 驗簽結果 | pass / fail |
| 處理結果 | 三種值：processed（成功處理）/ duplicated（重複推送已跳過）/ error（處理異常）|

三欄（廠商、本系統訂單號、廠商交易號）組合唯一，確保同一筆交易不重複處理。收到 Webhook 先寫入紀錄表；若唯一性衝突（重複推送），直接回 `1|OK`，不重複更新訂單。
...
- **工程師這邊需要注意：**
  1. **Webhook 冪等性**是最重要的安全設計，同一筆交易的 Webhook 可能因網路問題被重複推送，系統必須保證同一筆交易只處理一次（§8.5.1 紀錄表唯一性約束）
  2. Webhook 接收端必須在廠商限制時間內（最短 2 秒）回傳，因此發票開立、NT 通知等須異步執行，不可同步等待完成才回傳
...
- **方案金流/物流/電子發票規格已於 v1.2（2026/05/28）完整定案**，詳見 §2.3 方案對照表。進階電商包第三方金流可自選 6 種（13 家可選廠商清單已定案）；電子發票兩方案均內含綠界科技，另可加費串接 ezPay。
