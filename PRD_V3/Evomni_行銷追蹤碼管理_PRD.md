...
| v1.1 | 2026/05/28 | 導覽位置調整：頁面路徑從「全域設定 > 行銷追蹤碼管理」改為「行銷管理 > 行銷追蹤碼管理」；操作者定義為行銷人員，與全域設定（技術管理員）定位分離 | Una |
| v1.4 | 2026/06/02 | 依議題 2026-05-21-tracking-article-pageview-toggle §6.1：「文章頁發送事件」開關改為追蹤所有 CMS 文章詳情頁（不限 menu_id，依路由判斷）；待 CMS 維護者確認文章路由格式 | Una |
| v1.3 | 2026/06/02 | 依議題 2026-05-21-tracking-event-emitter-source §3：新增各 domain 埋點責任歸屬表；search 事件明確為追蹤注入層被動偵測（不歸任何單一 domain）；補充待確認：CMS 搜尋結果頁 URL 格式與 query string 參數名稱 | Una |
...
| 搜尋結果頁（追蹤注入層偵測，見下方說明）| search | Search | search_term（使用者查詢關鍵字，從 URL query string 取得）|
...
| 瀏覽文章頁（選填）| page_view | ViewContent（content_name: 文章標題）| 後台可設定是否對文章頁發送事件；「文章頁」以 CMS 文章詳情頁路由判斷，不限文章分類類型（menu_id）|

---

### 各事件 domain 埋點責任歸屬（依議題 2026-05-21-tracking-event-emitter-source 定案）

各事件的 dataLayer push 責任歸屬如下。工程師實作對應 domain 功能時，需確認觸發點有正確 push 事件；追蹤碼 PRD 為事件規格的唯一來源，各 domain PRD 不重複定義參數格式。

| 事件 | 負責 domain | 觸發時機 | 備註 |
| --- | --- | --- | --- |
| `view_item` | 商品中心 | 商品詳情頁載入時 | 參數規格見上方事件表 |
| `add_to_cart` | 購物車模組 | 點擊「加入購物車」按鈕時 | 含 LP 一頁式商店的加入購物車動作 |
| `begin_checkout` | 購物車模組 | 進入結帳表單（步驟 2）時 | |
| `purchase` | 訂單模組 | 付款成功頁載入時 | 含 LP 一頁式商店的訂單完成 |
| `search` | **追蹤注入層（被動偵測）** | 搜尋結果頁載入時 | 不歸任何單一 domain；由追蹤注入系統偵測 URL pattern，自動從 query string 取 `search_term` push；CMS 主程式不需改動任何邏輯 |
| `sign_up` | 會員認證模組 | 會員註冊完成時 | |
| `add_to_wishlist` | 會員前台 | 點擊「加入心願清單」時 | 進階電商包功能 |

> ⚠️ **待 CMS 主程式維護者確認（技術對齊，不影響 PM 決策）：**
> 追蹤注入層需要以下資訊才能正確偵測 `search` 事件：
> 1. 站內搜尋結果頁的 URL pattern（例：`/search`、`/?s=`、`/results?q=`）
> 2. 關鍵字的 query string 參數名稱（例：`q`、`s`、`keyword`）
> CMS 不需要改任何程式碼，僅需提供此技術資訊供追蹤注入層設定。
...
後台「行銷管理」→「行銷追蹤碼」→ 看到四個工具的設定卡片 → 依需求分別啟用並填入憑證 → 儲存 → 前台立即生效。
...
| 文章頁發送事件 | `<el-switch>` | — | 預設 OFF；開啟後所有 CMS 文章詳情頁皆觸發 page_view 事件，不限文章分類類型；追蹤注入層依 CMS 文章路由判斷（與 menu_id 無關）；說明文字（小字）：「適合有內容行銷策略的商家，開啟後可追蹤所有文章的瀏覽數據」 |

> ⚠️ **待 CMS 主程式維護者確認（技術對齊，不影響 PM 決策）：**
> 追蹤注入層需要 CMS 文章詳情頁的路由格式才能正確偵測。請提供：
> - 文章詳情頁的 URL pattern（例：`/post/{slug}`、`/article/{id}`、`/{menu_id}/{slug}` 等）
> - 確認所有文章類型（news / blog / faq 等）是否共用同一路由結構，或各有不同格式
> CMS 不需要改任何程式碼，僅需提供此資訊供追蹤注入層設定偵測規則。
...
**資料表：marketing_tracking_settings（行銷追蹤碼設定，每店一筆）**

| 欄位 | 業務說明 |
| --- | --- |
| id | 系統流水號（主鍵） |
| store_id | 所屬店鋪 ID（每店唯一） |
| ga4_enabled | GA4 是否啟用（開/關） |
| ga4_measurement_id | GA4 評估 ID（Measurement ID） |
| ga4_track_article | GA4 是否追蹤文章頁瀏覽事件（開/關） |
| fb_pixel_enabled | FB Meta Pixel 是否啟用（開/關） |
| fb_pixel_id | FB Pixel ID |
| fb_currency | FB 事件貨幣單位（TWD／USD／HKD，預設 TWD） |
| fb_capi_enabled | FB Conversion API 是否啟用（開/關） |
| fb_capi_token | FB CAPI Access Token（AES-256 加密儲存，不在前端暴露） |
| gtm_enabled | GTM 是否啟用（開/關） |
| gtm_container_id | GTM Container ID |
| created_at | 建立時間 |
| updated_at | 最後更新時間 |

**資料表：marketing_tracking_event_stats（事件觸發統計，每小時 batch 更新，非 real-time）**

| 欄位 | 業務說明 |
| --- | --- |
| id | 系統流水號（主鍵） |
| store_id | 所屬店鋪 ID |
| event_name | 事件名稱（如 PageView、Purchase 等） |
| stat_date | 統計日期 |
| trigger_count | 當日觸發次數 |
| updated_at | 最後更新時間 |
