...
| 版本 | 日期 | 修改內容 | 修改人 |
|------|------|----------|--------|
| v2.4 | 2026/05/28 | §8.5 資料庫補充規格：移除全部 CREATE TABLE SQL 語法，改為業務語言欄位需求表（Markdown 格式），共 7 張資料表 | Una |
| v2.3 | 2026/05/27 | §8.2 新增「金融欄位權限矩陣」：沿用現有 CMS RBAC 架構，新增「數據中心 > 金融欄位」權限類別（`analytics.view_revenue` / `analytics.view_margin` / `analytics.view_cost_price`），僅在電商方案啟用時顯示（決議 8-1）；§6.1 確認 GMV 不回扣退款快照政策（決議 8-2）；§7.3 RFM M 分數確認採全店歷史所有訂單分位數（決議 8-3） | Una |
...
| 文件狀態 | v2.3 — 新增金融欄位 RBAC 權限矩陣（電商方案條件顯示）；確認 GMV 快照不回扣退款政策；RFM M 分數確認採全店歷史分位數 |
| 作者 | Una |
...
| --- | --- | --- | --- |
| 總銷售金額（GMV）| SUM（已完成訂單金額）；每日快照後鎖定，退款不回扣歷史快照（決議 8-2）| `NT$ X,XXX,XXX` | 「已完成訂單的商品金額總和（不含取消訂單）。退款金額獨立呈現，不從歷史 GMV 快照中扣除。」|
...
graph TD
    ...
    C --> D3[M = 消費總金額\n依**全店歷史所有訂單**分位數計算 1-5 分（決議 8-3）\n新開店樣本少時仍穩定運作；滾動 365 天分位數等進階方案留待第二階段]
...
#### 8.2.1 金融欄位權限矩陣（決議 8-1）

**架構原則：** 沿用現有 CMS RBAC 架構，不新增 owner/operator 角色類型。在系統設定 > 權限矩陣頁面新增「數據中心 > 金融欄位」權限類別。

**新增權限項目：**

| 權限代號 | 說明 | 第一帳號預設 | 敏感程度 |
| --- | --- | --- | --- |
| `analytics.view_revenue` | 查看營業額（GMV、訂單金額）| 開啟 | 基礎，預設所有可設定角色開放 |
| `analytics.view_margin` | 查看毛利率 | 開啟 | 中等 |
| `analytics.view_cost_price` | 查看商品成本價 | 開啟 | 最敏感，需明確授權 |

**條件顯示規則：** 此「數據中心 > 金融欄位」權限類別**僅在商家開通電商方案時**才顯示於權限矩陣頁面。純 CMS 方案商家不顯示此類別（因不涉及電商金融欄位）。

**第一帳號（最高權限）：** 三項權限預設全部開啟，不可由其他帳號修改。其他帳號的權限由擁有者在權限矩陣頁面自由設定。
...
**資料表：analytics_daily_sales（每日銷售快照）**

| 欄位 | 業務說明 |
|------|----------|
| id | 流水號，主鍵 |
| date | 快照日期，不可重複 |
| gmv | 當日 GMV（成交總金額），預設 0 |
| order_count | 當日訂單數，預設 0 |
| aov | 當日平均訂單金額，預設 0 |
| completion_rate | 完成率（0.0000 ~ 1.0000），預設 0 |
| new_customer_count | 當日新客數，預設 0 |
| returning_customer_count | 當日回購客數，預設 0 |

**資料表：analytics_product_stats（商品分析快照，每日）**

| 欄位 | 業務說明 |
|------|----------|
| id | 流水號，主鍵 |
| date | 快照日期 |
| product_id | 對應商品 ID，與 date 組成唯一鍵 |
| pv | 商品頁瀏覽次數，預設 0 |
| uv | 商品頁不重複訪客數，預設 0 |
| add_to_cart_count | 加入購物車次數，預設 0 |
| order_count | 產生訂單數，預設 0 |
| gmv | 商品成交金額，預設 0 |

**資料表：member_rfm_scores（會員 RFM 分群，每日更新）**

| 欄位 | 業務說明 |
|------|----------|
| id | 流水號，主鍵 |
| member_id | 對應會員 ID，每位會員唯一一筆 |
| r_score | 最近購買時間分數（1–5，5 最近） |
| f_score | 購買頻率分數（1–5，5 最高頻） |
| m_score | 消費金額分數（1–5，5 最高，依各商店消費分位數計算） |
| rfm_total | RFM 三項加總（3–15） |
| segment | 分群標籤：`loyal`（忠誠客）、`potential`（潛力客）、`sleepy`（沉睡客）、`at_risk`（流失風險）、`lost`（已流失）|
| ltv | 會員累計消費金額（LTV），預設 0 |
| last_order_date | 最後一筆訂單日期（可空） |
| order_count | 歷史訂單數，預設 0 |
| total_spent | 歷史消費總額，預設 0 |
| calculated_at | 最後計算時間 |

**資料表：analytics_marketing_performance（行銷活動成效快照，每日）**

| 欄位 | 業務說明 |
|------|----------|
| id | 流水號，主鍵 |
| date | 快照日期 |
| campaign_type | 活動類型：`coupon`（折價券）、`fullorder`（滿額折）、`timelimited`（限時優惠）、`bundle`（組合優惠）、`journey`（自動化旅程）|
| campaign_id | 對應活動 ID，與 date + campaign_type 組成唯一鍵 |
| trigger_count | 活動觸發次數，預設 0 |
| conversion_count | 轉換（成交）次數，預設 0 |
| email_open_count | Email 開信次數，預設 0 |
| email_click_count | Email 點擊次數，預設 0 |
| gmv_attributed | 歸因至本活動的成交金額，預設 0 |
| discount_amount | 本活動產生的折扣總額，預設 0 |

**資料表：analytics_ad_spend（廣告花費設定，管理員手動填入）**

| 欄位 | 業務說明 |
|------|----------|
| id | 流水號，主鍵 |
| utm_campaign | UTM Campaign 名稱，與 spend_date 組成唯一鍵 |
| utm_source | UTM 來源（可空） |
| utm_medium | UTM 媒介（可空） |
| spend_date | 廣告花費對應日期 |
| spend_amount | 廣告花費金額 |
| created_by | 填入資料的操作員 ID（可空） |
| created_at | 建立時間 |
| updated_at | 最後更新時間 |

**資料表：analytics_cohort_monthly（同期群分析快照，每月計算一次）**

| 欄位 | 業務說明 |
|------|----------|
| id | 流水號，主鍵 |
| cohort_month | 會員加入月份（格式 YYYY-MM-01），與 analysis_month 組成唯一鍵 |
| analysis_month | 分析月份（格式 YYYY-MM-01） |
| cohort_size | 該期加入的會員總數 |
| returning_count | 在 analysis_month 內有購買行為的人數 |
| retention_rate | 回購率（returning_count ÷ cohort_size） |

**資料表：reward_receivable_daily_snapshot（應收餘額每日快照）**

> v2.2 新增，連動 Part4 v1.2 §6.6C / 優惠計算引擎 §7.4。寫入方主規格位於優惠計算引擎，本表為 Part5 視覺化資料來源。

| 欄位 | 業務說明 |
|------|----------|
| id | 流水號，主鍵 |
| snapshot_date | 快照日期，不可重複 |
| issued_amount | 該日發放回饋總額，預設 0 |
| recovered_amount | 該日成功扣回總額，預設 0 |
| voided_amount | 該日應收註銷總額，預設 0 |
| pending_balance | 該日結算後應收餘額，預設 0 |
...
- §8.2.1 新增「金融欄位權限矩陣」：沿用 CMS RBAC 架構，三項細粒度權限（`analytics.view_revenue` / `analytics.view_margin` / `analytics.view_cost_price`），僅電商方案開通時在權限矩陣中顯示此類別
- §6.1 GMV 欄位說明補充：退款不回扣歷史快照，退款金額獨立呈現（確認為最終政策）
- §7.3 RFM M 分數計算確認：採全店歷史所有訂單分位數，不採滾動 365 天（留待第二階段）
