...
| 版本 | 日期 | 修改內容 | 修改人 |
|------|------|----------|--------|
| v1.4 | 2026/06/03 | 依議題 2026-05-21-redirect-query-string-strategy §6.3 §7.2：前台 redirect 預設透傳原請求 query string（保留 UTM 等廣告歸因參數）；合併邏輯由工程師自行決定 | Una |
| v1.3 | 2026/06/03 | 依議題 2026-05-21-cms-page-vs-basic-data-naming：統一技術命名——`page_slug_history` 改為 `web_menu_item_publish_history`（沿用既有 CMS 命名慣例）；§2.1 `pages table` 更正為 `basic_data / article`；UI 仍稱「頁面」不變 | Una |
| v1.2 | 2026/05/28 | §8.3 DB Schema 建議：移除 CREATE TABLE / INSERT INTO SQL 語法，改為業務語言欄位需求表（Markdown 格式） | Una |
| v1.1 | 2026/05/28 | 14-2 定案：分類列表頁（文章分類 / 商品分類）SEO 保護納入本階段執行（§6.2.B 觸發條件更新，§8.4 移除分類頁暫不涵蓋的限制說明）；14-3 定案：相關警告均為「提示性警告」，不阻擋商家操作（§6.2 更新確認彈窗設計）| Una |
...
| 文件狀態 | v1.4 — 前台 redirect 預設透傳 query string（UTM 保留）；合併邏輯由工程師自行決定 |
...
3. 技術層面建立 `web_menu_item_publish_history` 與 `redirects` 表支援 redirect 機制
...
| CMS 頁面管理（`basic_data / article`） | 讀取頁面發布狀態；顯示 callout |
...
mindmap
  root((CMS 選單 SEO 保護))
    ...
    移除節點保護
      ...
      redirect 目的地選擇器
    301 Redirect 機制
      web_menu_item_publish_history 記錄
      redirects 表維護
      前台 routing API 比對
      重新導向管理後台入口
    雙系統路由差異標示
...
| 選單節點類型為「文章分類 / 產品分類」 | ✅ **觸發（14-2 定案：本期納入）** — 分類列表頁 URL 已被搜尋引擎收錄的機率高，移除選單節點時同樣觸發警告確認彈窗，保護 SEO 積累 |

> **邏輯說明：** 24 小時門檻代表這個 URL 已有足夠時間被搜尋引擎爬蟲發現並收錄，因此需要保護。門檻值可由後台全域設定調整（預設 24 小時），見 §6.4。

> **⚠️ 重要設計原則（14-3 定案）：** 本節所有警告均為**提示性警告**，不阻擋商家操作。商家閱讀警告後可選擇「確認移除（不設定轉址）」繼續執行，系統尊重商家決策。設計理由：強制阻擋會讓商家無法正常整理網站架構，且部分 URL 本就不需保護（如測試頁、活動已結束的臨時頁）。
...
消費者或搜尋引擎訪問舊 URL → 前台 routing API 在返回 404 之前先查詢 `redirects` 表 → 若命中，回傳 HTTP 301 並帶上目的 URL（含原請求的 query string）→ 瀏覽器自動跳轉至新頁面。

> **Query String 透傳行為**（依議題 2026-05-21-redirect-query-string-strategy 決議）：前台 redirect 預設透傳原請求的 query string 至目的 URL，以保留 UTM 等廣告歸因參數的完整性。
>
> **業務理由**：redirect 不應切斷使用者的來源脈絡。UTM 斷鏈會導致廣告歸因失準，商家無法正確評估廣告投入效益；預設透傳為業界標準做法。
>
> **範例**：消費者訪問 `/old-page?utm_source=fb&utm_campaign=summer` → 命中 redirect → 最終跳轉至 `/new-page?utm_source=fb&utm_campaign=summer`
>
> **合併策略**：若目的 URL 本身已含 query string，合併邏輯由工程師依技術判斷實作，不在 PM 規格範圍內。
...
graph TD
    ...
    F --> G{商家選擇？}
    G -- 取消 --> H[關閉彈窗，不執行操作]
    G -- 確認移除（不設定轉址）--> I[DELETE menu_item\n寫入 web_menu_item_publish_history]
...
graph TD
    ...
    F -- 是 --> G[將原請求 query string\n附加至目的 URL（透傳）]
    G --> H[回傳 HTTP 301\nLocation: 目的 URL + query string]
    F -- 否 --> I[回傳 404]
```
...
**資料表：web_menu_item_publish_history（頁面歷史路由記錄）**

| 欄位 | 業務說明 |
|------|----------|
| id | 流水號，主鍵 |
| page_id | 對應的 `basic_data.id`（CMS 頁面 ID） |
| slug | 完整前台路徑，例如 `/about/brand-story` |
| menu_item_id | 產生此路徑的選單節點 ID（可空） |
| published_at | 此路徑首次公開發布時間（可空） |
| removed_at | 從選單移除的時間（可空） |
| created_at | 建立時間 |

**資料表：redirects（301 轉址規則）**

| 欄位 | 業務說明 |
|------|----------|
| id | 流水號，主鍵 |
| source_url | 來源路徑（相對路徑，如 `/old-page`），不可重複 |
| target_url | 目的路徑或完整 URL |
| redirect_type | 轉址類型，目前固定 301 |
| created_by | 建立來源：`manual`（管理員手動新增）、`menu_removal`（系統因選單移除自動建立），預設 `manual` |
| created_at | 建立時間 |
| updated_at | 最後更新時間 |
...
| key | 預設值 | 型別 | 業務說明 |
|-----|--------|------|----------|
| menu_removal_warning_threshold_hours | 24 | integer | 選單節點移除前觸發警告的門檻小時數（頁面已發布超過此時數才顯示 SEO 警告） |

### 8.4 已知限制

- **既有 404 頁面無法回溯**：`web_menu_item_publish_history` 僅記錄本機制上線後的行為，上線前已失效的舊 URL 無法自動補建 redirect 規則，需商家手動至「重新導向管理」補設
- **分類列表頁保護**：✅ 已納入本期（14-2 定案），觸發條件與單頁 CMS 頁面相同，見 §6.2.B
...
- 工程師後端需要新建兩張表：`web_menu_item_publish_history` 和 `redirects`；前台 routing API 在查無路由時新增一層 `redirects` 比對
