...
| 版本 | 日期 | 修改內容 | 修改人 |
|------|------|----------|--------|
| v1.5 | 2026/05/28 | §6.1.D DB 規格、§8 DB 補充：移除 CREATE TABLE SQL 語法，改為業務語言欄位需求表（Markdown 格式） | Una |
| v1.4 | 2026/05/28 | 13-2 定案：方案到期後三階段鎖定機制（14 天寬限期全功能 + 每日 Banner→讀寫限制＋固定 Banner→無全面封鎖），新增 §6.1.G；13-3 定案：到期通知採群組信箱；13-5 定案：不再顯示自定義 CTA，改為統一「預約方案諮詢」連結至 https://www.webtech.com.tw/contact（§6.3 更新）；13-6 定案：本階段不提供降級功能 | Una |
| v1.3 | 2026/05/27 | 更新商品管理顯示上限數字：啟航方案 1,000 件 / 進階電商包 2,500 件（方案狀態頁與方案比較頁均已更新） | Una |
| v1.2 | 2026/05/27 | 修正方案功能標記錯誤（1-11 Phase 審查）：「一頁式商店」與「KOL 分潤管理」從電商啟航方案包含清單移出，正確歸類至進階電商包專屬（兩處畫面：方案狀態頁 + 方案比較頁均已更正） | Una |
...
| 文件狀態 | v1.4 — 三階段到期鎖定機制 / 降級政策 / 諮詢 CTA URL 定案 |
...
| 已到期 | 詳見 §6.1.G 三階段鎖定機制 | 到期當日通知信 | 「{方案名稱} 已到期，請盡速聯絡營運輔導顧問以恢復服務」 |
...
#### G. 方案到期後三階段鎖定機制（13-2 定案）

方案到期後，**不立即全面封鎖**，採三階段漸進式處理，降低商家損失並保有緩衝時間：

**第一階段：寬限期（到期後 Day 1 ～ Day 14）**

| 項目 | 說明 |
|------|------|
| 功能狀態 | 全功能正常運作（包括接單、新增商品、修改設定）|
| 通知方式 | 後台每日顯示提醒 Banner（非阻擋式）|
| Banner 文字 | 「您的方案已於 {日期} 到期，目前為 14 天寬限期（剩餘 N 天）。請盡速聯絡顧問續約，避免功能受限。[預約方案諮詢]」|
| CTA | 連結至 https://www.webtech.com.tw/contact |

**第二階段：讀寫限制（寬限期結束後，功能受限直到續約）**

| 項目 | 說明 |
|------|------|
| **可繼續使用的功能** | ✅ 查詢歷史訂單 / 匯出訂單資料 / 處理退換貨與退款 |
| **受限（不可執行）的功能** | ❌ 新增商品 / 接受新訂單 / 修改全域設定 / 修改金物流設定 |
| 通知方式 | 後台頂部固定橫幅（Sticky Banner，阻擋感但不遮蔽整頁）|
| Banner 文字 | 「您的方案已到期，目前處於唯讀模式。可查詢訂單、匯出資料與處理退款，但新訂單將無法成立。[預約方案諮詢]」|
| 前台消費者 | 前台商店關閉，消費者訪問時顯示「商店暫時關閉，敬請期待」（可自訂文字）|

**第三階段：無全面封鎖**
- 本系統**不設計「完全封鎖後台」機制**
- 商家在第二階段始終保有查詢、匯出、退款能力，避免因方案到期而無法處理消費者服務糾紛
- 如需恢復全功能，商家於後台點擊「預約方案諮詢」CTA，走人工續約流程

**通知群組信箱（13-3 定案）：**
- 到期提醒通知信寄送至商家帳號設定的**群組信箱**（如 `store@example.com`），確保團隊成員均可收到
- 若商家未設定群組信箱，寄至 CMS 系統設定的主要管理員 Email

---

#### D. DB 規格

**資料表：client_subscriptions（商家方案訂閱表）**

> 若 CMS 尚未有類似結構則新增；工程師可依技術判斷調整欄位實作。

| 欄位 | 業務說明 |
|------|----------|
| id | 流水號，主鍵 |
| client_id | 對應商家帳號 ID |
| plan_type | 方案類型，可選值：`hosting_basic`（基礎主機方案）、`hosting_enterprise`（企業主機方案）、`ai_basic`（AI 基礎版）、`ai_advanced`（AI 進階版）、`ecommerce_starter`（電商啟航方案）、`ecommerce_advanced`（進階電商包）|
| status | 訂閱狀態：`active`（生效中）、`expired`（已到期）、`cancelled`（已取消），預設 `active` |
| started_at | 方案生效日期 |
| expires_at | 方案到期日期 |
| renewed_at | 最近一次續約日期（可空） |
| activated_by | Evomni 內部執行開通的操作人員帳號（可空） |
| notes | 備注（可空） |
| created_at | 建立時間 |
| updated_at | 最後更新時間 |
...
| `Evomni_Part2_商品中心_PRD.md` 0元商品 | 「需升級進階電商包」文字 | 引用 `<UpgradeLockBanner>` |
...
│                                                               │
│  ❓ 不確定哪個方案適合您？[預約方案諮詢]                      │
│     連結至 https://www.webtech.com.tw/contact               │
└───────────────────────────────────────────────────────────────┘
```
...
- `[預約方案諮詢]`（底部）：`<el-link>`，連結至 https://www.webtech.com.tw/contact，在新分頁開啟（13-5 定案）
...
- 從進階電商包降回電商啟航方案的操作流程，本期不設計、不實作
- 商家如有降級需求，需透過人工流程與 Evomni 業務人員協商
- 設計理由：降級涉及資料相容性（如超出啟航方案商品數量上限的商品處理）和計費退款問題，需逐案評估，自動化降級規則目前尚未定義
...
**資料表：upgrade_requests（升級詢問紀錄表）**

| 欄位 | 業務說明 |
|------|----------|
| id | 流水號，主鍵 |
| client_id | 提出升級詢問的商家 ID |
| current_plan | 商家目前方案：`ecommerce_starter`（電商啟航方案）、`ecommerce_advanced`（進階電商包）|
| target_plan | 商家欲升級的目標方案：同上可選值 |
| contact_name | 商家填寫的聯絡人姓名 |
| contact_phone | 商家填寫的聯絡電話 |
| preferred_contact_time | 偏好聯絡時段（可空，商家自填文字） |
| notes | 其他備注（可空） |
| status | 處理狀態：`pending`（待處理）、`in_progress`（處理中）、`completed`（已完成）、`cancelled`（已取消），預設 `pending` |
| handled_by | Evomni 負責處理的內部人員帳號（可空） |
| handled_at | 處理完成時間（可空） |
| payment_note | 人工確認收款備注（可空） |
| created_at | 建立時間 |
| updated_at | 最後更新時間 |
