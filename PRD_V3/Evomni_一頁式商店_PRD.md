...
| v2.3 | 2026/05/27 | 釐清「一頁式商店」命名定義：「一頁」指單一轉換目標的活動單元（非字面一頁），內部 3 個畫面（銷售頁/規格加購/結帳）為操作流程步驟，非獨立頁面數；商家可建立多組一頁式商店；移除易誤解的「輕量 3 頁」說法 | Una |
| v2.4 | 2026/05/27 | 5-1 確立 LP 優惠來源雙軌架構（主站行銷活動直套用 + LP 獨立設定，衝突以 LP 優先；點數折抵需登入）；5-2 KOL Token 移除永久有效選項，上限 12 個月；5-3 KOL 分潤改以實際支付金額（扣除折扣/券/點數，不含運費）為計算基準；5-4 LP 訪客棄單挽回確認列 Phase 2，v1.0 僅統計棄單數 | Una |
| v2.7 | 2026/06/02 | 依議題 2026-05-20-lp-guest-email-abandon-journey §6.9C：LP 訪客棄單挽回旅程從「Phase 2 不實作」改為本期正式支援；新增觸發條件、Email 模板（訪客路徑）、Token 機制、合規三要件（告知 + checkbox 未預設勾選 + 同意紀錄）；發送時機固定 1 小時，本期不附券 | Una |
| v2.6 | 2026/05/29 | §6.12 補充密碼設定連結過期處理：`pending_activation` 帳號可透過「忘記密碼」自行重設，無需聯繫商家；通知信 4-4 同步更新 | Una |
| v2.5 | 2026/05/28 | §8.5 DB Schema 補充：移除 SQL 語法，改以業務語言欄位需求表（Markdown 表格）呈現 | Una |
| v1.0 | 2026/04/27 | 初稿建立：3 步驟操作流程（銷售頁 → 規格加購 → 結帳）、倒數計時、基礎統計 | Una |
...
| 文件狀態 | **v2.7** — §6.9C LP 訪客棄單挽回旅程本期正式支援（含合規三要件）|
| 對應方案 | 進階電商包 ✅（電商啟航方案 ❌）|
...
**三大核心使用場景：**
- **廣告投放型品牌（保健食品、美妝、功能性商品）**：FB/IG 廣告大量投放，需要單一 Landing Page 承接流量、快速結帳，不希望消費者跑去主站分心
...
| 台灣食品類客戶需要低溫宅配 | 繼承主站商品的溫層屬性，結帳自動套用低溫物流規則 |

**系統相依性：**

| 模組 | 用途 |
| --- | --- |
| Part 2 商品中心 | 商品資料來源（庫存/規格/價格/溫層屬性）；訂單建立時原子扣庫存 |
| Part 3 訂單管理 | 結帳完成後建立訂單、電子發票、發信系統通知；KOL 分潤標籤寫入 |
| 全域設定 > 金物流串接 | 結帳頁金流選擇；溫層物流費率計算（繼承主站規則） |
| Part 4 行銷活動 | 加價購推薦商品來源；購物車放棄追蹤數據提供給挽回旅程 |
| 媒體庫 | 商品圖、Banner 圖、影片；自動 WebP 轉換 |
...
| 一頁式商店設定 | 低溫物流支援 | 食品類商品正確運費 | 主推商品有溫層屬性時，結帳頁自動顯示低溫宅配選項，並套用主站溫層運費規則 | 消費者、商家管理員 |
| KOL 分潤模組 | KOL 設定 | 建立 KOL 合作關係 | 在一頁式商店層級設定：KOL 姓名/暱稱、分潤比率（%）、連絡 Email；系統自動建議以 KOL 名稱為 Slug | 商家管理員 |
| KOL 分潤模組 | 分潤計算報表 | 結算 KOL 業績 | 統計該 KOL 一頁式商店的：訂單數、成功訂單總金額（扣除退款）、應付分潤金額（= 實際支付商品金額（不含運費，扣除折扣/券/點數）× 分潤率）；可依日期範圍篩選 | 商家管理員 |
| KOL 分潤模組 | KOL 報表分享連結 | 讓 KOL 自行查看業績 | 產生含 Token 的唯讀分享連結（有效期 30/90/180/365 天或自訂，上限 12 個月，無永久有效選項）；KOL 只能看到自己那個一頁式商店的數據，無法存取其他一頁式商店或後台 | 商家管理員、KOL |
...
| 前台銷售頁 | 固定底部 CTA Bar | 永遠可見的購買入口 | 全程固定在頁面底部，顯示商品名稱 + 價格 + 購買按鈕 | 消費者 |
| 前台規格/加購頁 | 規格選擇（Bottom Sheet）| 選擇商品規格 | 最多 3 層規格；無庫存灰色顯示；未選規格時防呆提示 | 消費者 |
| 前台規格/加購頁 | 加價購推薦 | 提升客單價 | 2-4 件加購商品橫向捲動；來自行銷中心加價購設定 | 消費者 |
...
| 購物車放棄追蹤 | 銜接挽回旅程 | 自動行銷跟進（Phase 2） | **v1.0 不實作**：僅統計棄單數，不觸發挽回旅程。Phase 2 評估：若消費者已留下 Email，推入行銷中心挽回旅程佇列（需評估 ROI 與合規性）| 商家管理員、消費者 |
...
| US-07 | 消費者 | 身為消費者，我想要在選完規格後看到推薦的加購商品，以便一次買齊需要的配件。 |
...
| 主推商品 | `<el-select>` 遠端搜尋 | 必填；選後自動帶入商品基本資料；若商品有溫層屬性，顯示提示「⚠️ 此商品為低溫商品，結帳時將自動套用低溫物流規則」|
| 網址 Slug | `<el-input>` | 必填；格式 `/^[a-z0-9\-_]{3,50}$/`；唯一性即時驗證；自動從商品名稱產生建議 Slug（轉小寫、空格換 `-`）；重複時 Error：「此網址已被使用，請換一個」|
| SEO 標題 | `<el-input>` | 選填；最多 60 字；Placeholder：「預設使用商品名稱」；下方顯示字數計數器 |
| SEO 描述 | `<el-input type="textarea">` | 選填；最多 160 字 |
| OG 分享圖片 | `<el-upload>` | 選填；建議尺寸 1200×630px；上傳後呼叫媒體庫 API；Tooltip：「此圖片在 LINE / FB 分享時作為預覽圖，建議放商品主視覺」|
...
[設定 KOL 分潤]  <el-collapse>
  ...
  Info Tooltip：「分潤金額 = 實際支付商品金額（商品小計扣除折扣/優惠券/點數折抵，不含運費，扣除退款）× 分潤比率。系統僅計算，不自動轉帳。」
```
...
| 版型名稱 | 適用場景 | 預設包含區塊 |
| --- | --- | --- |
| 單品爆款版 | 廣告投放、保健食品、美妝 | Hero 大圖 + 賣點卡片 × 3 + 商品圖輪播 + 消費者評論 + 倒數計時 + 固定底部 CTA |
| KOL 合作版 | 網紅推薦、開箱團購 | Hero 影片嵌入 + KOL 推薦文字區 + 賣點卡片 × 4 + 消費者評論 + 固定底部 CTA |
| 限時搶購版 | 限時活動、會員專屬 | 倒數計時（置頂大字）+ Hero 圖 + 賣點卡片 × 3 + 商品圖展示 + 固定底部 CTA（含剩餘庫存提示）|
...
| 商品圖片區 | 瀑布式或輪播圖片展示區；圖片自動呼叫媒體庫 API 轉 WebP | Page 1 中段 |
| 消費者評論 | 星等 + 評論文字 + 評論者名稱；最多 10 則；支援評論圖片 | Page 1 下段 |
| 倒數計時 | 活動結束倒數 `HH:MM:SS`；結束後行動由步驟 1 設定控制 | Page 1 任意位置 |
| 固定底部 CTA Bar | 始終固定於頁面底部；顯示商品名稱 + 售價 + 立即購買按鈕 | 全頁面固定 |
| 文字區塊 | 富文本段落、標題；適合產品詳細說明 | Page 1 中段 |
| 圖文並排 | 左圖右文或左文右圖的雙欄佈局 | Page 1 中段 |
| 分隔線 / 空白間距 | 版面調整用 | 任意位置 |

> **FAE 說明：** 商品規格選擇（Page 2）和結帳（Page 3）是系統自動接續的功能頁，不在 GrapeJS 編輯器的設計範圍內——消費者點擊任何「立即購買」或「固定底部 CTA」按鈕後，系統統一跳轉至規格選擇 Bottom Sheet，接著進入結帳頁。這兩個頁面的 UI 由系統統一管理，確保結帳體驗一致。
...
| 欄位 | 元件 | 說明 |
| --- | --- | --- |
| 是否顯示加購區塊 | `<el-switch>` | 預設 OFF；Tooltip：「在消費者選完主商品規格後，推薦搭配商品可有效提升客單價 15–30%」|
| 加購區塊標題 | `<el-input>` | 選填；預設「搭配更划算」|
| 加購商品（最多 4 件）| `<el-select>` 遠端搜尋 × 可新增 | 從行銷中心「產品加購」活動中選擇已設定的加購商品；含加購價和原價 |
...
- 說明文字：「金額計算基準：實際支付商品金額（商品小計扣除折扣、優惠券、點數折抵後，不含運費；退款訂單已扣除）× 分潤比率」
...
商家管理員點擊分享連結 → 選擇有效期 → 複製連結 → 傳給 KOL → KOL 用連結查看自己的業績
...
此連結的 KOL：[KOL 姓名]
有效期設定：  ○ 30 天  ○ 90 天  ● 180 天  ○ 365 天  ○ 自訂到期日（上限 12 個月）
...
> **設計說明：** 不提供「永久有效」選項。KOL 離職或合作結束後舊 Token 不應持續有效，12 個月為最長上限，保護商家數據安全。自訂到期日若超過 12 個月，系統自動鎖定為當日起算 12 個月後到期並提示：「到期日上限為 12 個月，已自動調整」。
...
[底部固定 CTA Bar]（position: fixed; bottom: 0; z-index: 999）
  商品縮圖（40×40px）+ 商品名稱（最多 1 行截斷）+ 售價 + [立即購買]（120px, 44px）
...
[加購推薦區（若啟用）]
  橫向捲動卡片列表
  每張卡片：商品圖（80×80）+ 名稱（最多 2 行）+ 劃除原價 + 加購價 + [+加購]
...
[物流選擇]
  若主推商品有溫層屬性（temperature ≠ 'normal'）：
    → 自動過濾顯示「低溫宅配」物流選項
    → 顯示提示：「此商品需低溫配送，請選擇低溫宅配服務」
    → 非低溫物流選項不顯示（避免選錯）
  否：顯示所有可用物流
...
- 低溫商品若無可用低溫物流廠商（商家未設定）：顯示警告「此商品需低溫配送，但您尚未設定低溫物流，請至全域設定 > 金物流串接 > 物流與運費 設定」，且該一頁式商店停用發布按鈕
- 提交時若收件資訊有誤：欄位邊框變紅 + 欄位下方顯示錯誤文案（不使用 Alert 彈窗）

---

### 6.7.1 優惠來源架構（LP 雙軌優惠機制）

LP 結帳頁支援兩種優惠來源，可同時並存。衝突時以 LP 獨立設定優先。

#### A. 來源 A — 套用主站行銷活動

- 主站行銷中心設定的下列優惠類型，可直接對 LP 訂單生效，商家**無需在 LP 重複設定**：
  - 分類型活動
  - 全館活動
  - 滿額折扣
  - 滿額贈
  - 組合優惠
- 消費者進入 LP 後，系統自動依主推商品所屬分類比對當前有效活動，符合條件即觸發

#### B. 來源 B — LP 獨立優惠設定

- 商家可在 LP 層級單獨設定以下優惠，**僅對該 LP 生效，不影響主站其他頁面**：
  - 專屬優惠券
  - 滿額折扣
  - 滿額贈
  - 組合優惠
...
| 情境 | 處理方式 |
| --- | --- |
| 主站活動與 LP 獨立設定同時有效 | **LP 獨立設定優先**，不疊加主站同類型活動 |
| 主站多個活動同時適用（LP 無獨立設定） | 依主站行銷中心的活動優先順序規則處理 |
| LP 獨立設定多個同類型優惠 | 依 LP 優惠設定的優先順序（同主站規則） |

#### D. 點數折抵規則

- 點數折抵功能**需會員登入後方可使用**
- 未登入的訪客結帳頁：**不顯示點數折抵入口**（直接隱藏，非 disabled 灰色）
- 已登入會員：顯示「使用點數折抵」區塊，顯示可用點數餘額，點數折抵金額即時計算至訂單明細

> **工程師說明：** 優惠來源的判斷由後端 API 於訂單試算（`POST /lp/{id}/calculate`）時統一處理，前端不做優惠邏輯計算。LP 獨立設定優先的規則在試算 API 中判定，確保前後端邏輯一致。
...
> **依議題 2026-05-20-lp-guest-email-abandon-journey 決議（2026/06/02）：** 本期正式支援 LP 訪客棄單挽回 Email 旅程。LP 流量主要來自廣告，訪客在結帳頁留下 Email 屬高意圖行為，黃金窗口 30–60 分鐘；若不觸發挽回等於放棄已買到的廣告流量，對商家 ROAS 影響直接。
...
| 條件 | 說明 |
| --- | --- |
| Email 已提交 | 訪客於 Page 3（結帳頁）成功送出 Email 欄位（非單純輸入，需經表單送出動作） |
| 已取得同意 | 訪客已勾選同意接收通知的 checkbox（見下方合規規格） |
| 60 分鐘未付款 | Email 提交後 60 分鐘內未產生付款成功事件（後端排程判定，不需偵測頁面關閉行為） |
| 冷卻期未重複 | 同一 Email × 同一 LP，7 天冷卻期內未觸發過挽回信 |

**不觸發情況：**
- 訪客未提交 Email 即離開
- 訪客已登入為會員（改走行銷中心標準 member 挽回旅程）
- 同一 Email 在 7 天冷卻期內已觸發過本 LP 的挽回信

**發送時機：** Email 提交後 **1 小時**（獨立設定，不沿用行銷 PRD 的通用 N 小時設定；LP 廣告流量購買意圖冷卻快，需更短窗口）

---

**▍Email 模板（訪客路徑）**

**信件主旨**（後台可設定，預設）：「🛒 您有商品還沒結帳！」
...
[CTA 按鈕]
  [立即結帳] → 含 token 的一次性結帳連結（有效期 48 小時）

[警示文字]（小字）
  此連結有效期為 48 小時，過期後需重新至商店選購
...
**與會員路徑差異：**
- `{會員姓名}` → 「您好」（訪客無姓名資料）
- 不附加優惠券（本期訪客路徑不支援；LP 雙軌優惠規則仍正常運作於結帳頁，訪客點連結返回後照常享有）
- 不發送 LINE 訊息（訪客無 LINE 綁定）

---

**▍Token 機制（共用 guest_email token 架構）**

- Token 格式：UUID v4
- 識別依據：`guest_email`（`member_id` 欄位可為空）
- 儲存位置：`cart_recovery_tokens` 表，新增欄位需求：`guest_email`、`lp_id`、`consent_record_id`
- 有效期：48 小時；使用後標記已使用，不可重複觸發
- 消費者點擊連結：預填 Email 欄位 + 還原所選商品，直接進入結帳確認步驟

---

**▍合規規格（個資法三要件，缺一不可）**

> 以下三項必須同時實作，缺任一項不得觸發挽回旅程。

**1. 告知用途**

Email 欄位正下方需顯示告知文字（字級可小一號，但必須清楚可見，不得藏在隱私政策連結後方）：

> 「您的 Email 將用於寄送訂單確認及購物提醒，包含未完成結帳的提醒通知。詳見[隱私權政策]。」
>
> （「隱私權政策」需為可點擊連結，並連至實際頁面）

> ⚠️ **待法務確認**：上述告知文字為草稿版，正式上線前需請法務審閱措辭是否符合個資法要求。

**2. 取得同意（checkbox）**
- 預設：**未勾選**（不可預設勾起）
- 勾選文字（草稿版）：

  > 「我同意接收來自本商店的訂單通知與購物相關提醒（包含未完成結帳的提醒信），電子郵件地址僅用於上述目的。」

  括號內明確點出「未完成結帳提醒」，符合個資法對具體用途告知的要求，不能僅寫模糊的「行銷通知」。

  > ⚠️ **待法務確認**：上述 checkbox 文案為草稿版，正式上線前需請法務審閱措辭是否符合個資法要求。

- 未勾選者：不觸發棄單挽回信（訂單確認信不受影響，照常發送）

**3. 同意紀錄可查**
後端須記錄以下資訊以備爭議舉證：
- Email 地址、同意時間（timestamp）、頁面來源（LP ID）、聲明版本識別碼
- 儲存於 `lp_consent_records` 表（或擴充現有同意紀錄機制）
- `cart_recovery_tokens.consent_record_id` 外鍵關聯此紀錄
- 聲明版本識別碼：初版以日期字串為準（如 `v20260602`），法務確認正式文案後更新版本號

---

**▍棄單事件記錄**

- 棄單事件仍寫入 `lp_events`（`event_type = 'checkout_abandoned'`），供漏斗統計
- 已觸發挽回信的紀錄寫入 `marketing_triggers`（`journey_type = 'lp_guest_recovery'`，`guest_email` 識別）
- 統計頁放棄追蹤卡片新增：**挽回信已發送數**、**挽回轉換數**（點擊連結後完成付款）
...
| 平均客單價 | GMV / 完成購買次數 | 加購商品有效的話此數字會高於主商品售價 |
...
| 項目 | 規格 |
| --- | --- |
| 計算基準 | 實際支付商品金額（商品小計 − 折扣 − 優惠券 − 點數折抵） |
| 不含項目 | 運費（不論物流費率高低，均不計入分潤基礎） |
| 公式 | `KOL 分潤金額 = 實際支付商品金額 × 分潤比率` |
| 退款處理 | 退款訂單分潤記錄標記 `status = refunded`，分潤報表自動扣除 |
| 市場對齊 | 與 SHOPLINE 計算基準一致，避免商家在讓利折扣之外再承擔原始定價的分潤成本 |

> **合約注意事項（商家自行負責）：** KOL 分潤以「實際支付商品金額」為基準，當商家對 LP 設定大幅優惠券或折扣時，KOL 的分潤基礎金額將同步降低。建議商家在 KOL 合作合約中明確標註此計算方式，避免結算爭議。
...
#### one_page_shops 資料表（新增 KOL 欄位）

| 欄位 | 業務說明 |
| --- | --- |
| kol_name | KOL 名稱，選填 |
| kol_email | KOL 聯絡 Email，選填 |
| kol_commission_rate | 分潤比率，數值 0–100，選填 |
| kol_report_token | 報表分享連結的唯一識別碼（UUID v4），選填 |
| kol_report_token_expires_at | 報表分享連結到期時間，選填 |

#### lp_kol_commissions（KOL 分潤記錄）

| 欄位 | 業務說明 |
| --- | --- |
| id | 主鍵，自動遞增 |
| one_page_shop_id | 所屬一頁式商店，關聯 one_page_shops |
| order_id | 對應訂單，關聯 orders |
| order_amount | 本次計算分潤的訂單金額（不含運費） |
| commission_rate | 套用的分潤比率（快照，防止事後改率影響舊紀錄） |
| commission_amount | 實際分潤金額 |
| status | 分潤狀態：`pending`（待結算）、`settled`（已結算）、`refunded`（已退款） |
| created_at | 記錄建立時間 |

#### lp_events（一頁式商店行為事件，漏斗追蹤）

| 欄位 | 業務說明 |
| --- | --- |
| id | 主鍵，自動遞增 |
| one_page_shop_id | 所屬一頁式商店 |
| session_id | 前端 Session 識別碼，用於串連同一訪客的多個事件 |
| event_type | 事件類型：`page_view`（瀏覽頁面）、`cta_click`（點擊主要行動按鈕）、`spec_open`（開啟規格選擇）、`cart_complete`（完成加入購物車）、`checkout_start`（進入結帳） |
| utm_source | 廣告來源（如 facebook、google） |
| utm_medium | 廣告媒介（如 cpc、social） |
| utm_campaign | 廣告活動名稱 |
| utm_content | 廣告素材識別 |
| utm_term | 廣告關鍵字 |
| created_at | 事件發生時間 |
...
**members 資料表 status 欄位狀態值（確認包含以下值）：**

| 狀態值 | 業務說明 |
| --- | --- |
| `active` | 正常啟用的會員 |
| `pending_activation` | 訪客結帳後系統自動建立的帳號，等待消費者設定密碼 |
| `suspended` | 已停用的帳號 |

**contacts 資料表（CRM 聯絡人，新增轉換記錄欄位）：**

| 欄位 | 業務說明 |
| --- | --- |
| converted_to_member_id | 若該聯絡人已轉換為會員，對應 members 的 id（選填；會員刪除時設為 NULL） |
| converted_at | 轉換為會員的時間（選填） |
...
> **密碼設定連結過期處理：** `{set_password_link}` 有效期 72 小時。連結過期後，消費者**無需聯繫商家**，可直接前往登入頁點選「忘記密碼」自行重新設定密碼；`pending_activation` 狀態的帳號支援忘記密碼流程，密碼設定成功後帳號狀態自動轉為 `active`。
...
| **商品說明展開提示文字** | 「查看完整說明 ↓」 | 商品說明折疊收合時的展開按鈕 | 15 字 |
...
**v2.4 變更要點（2026/05/27）：**
1. **LP 優惠來源雙軌架構（§6.7.1）：** 主站行銷活動可直接套用至 LP，同時支援 LP 獨立優惠設定；衝突時 LP 獨立設定優先；點數折抵需登入，未登入訪客不顯示入口
2. **KOL Token 有效期（§6.4）：** 移除「永久有效」選項，有效期限 30 天 / 90 天 / 180 天 / 365 天 / 自訂（上限 12 個月）
3. **KOL 分潤計算基準（§6.3、§7.1）：** 改為實際支付商品金額（扣除折扣/券/點數，不含運費），與 SHOPLINE 市場標準一致
4. **LP 訪客棄單挽回本期正式支援（§6.9C）：** 觸發條件為「結帳頁 Email 提交後 60 分鐘未付款 + 勾選同意 checkbox」；發送時機 1 小時（獨立設定）；Token 共用 guest_email 機制；本期不附券；合規三要件（告知 + 未預設勾選 + 同意紀錄）缺一不可

**待工程師確認：** §6.7.1 的優惠試算 API（`POST /lp/{id}/calculate`）需確認是否已涵蓋 LP 獨立優惠優先邏輯，以及 LP 獨立優惠設定的後台入口開發時程。
...
- **工程師需要注意：**
  ...
  2. **頁面設計沿用現有 GrapeJS 編輯器**；步驟 2 的電商區塊（Hero、倒數計時、規格選擇觸發）是電商專屬的預建區塊，需要與現有 GrapeJS 整合開發；商品規格選擇（Page 2）和結帳（Page 3）不在 GrapeJS 畫布範圍內，由系統統一接管
...
- **§6.12 通知信（工程師重點）：** 5 種信件均透過發信系統發送；各商店獨立維護信件範本，不繼承主電商通知設定；`{set_password_link}` 有效期硬性 72 小時；連結過期後消費者可透過前台「忘記密碼」功能自行重設，`pending_activation` 帳號支援此流程，密碼設定成功後帳號狀態自動轉為 `active`，無需重新觸發或聯繫商家
