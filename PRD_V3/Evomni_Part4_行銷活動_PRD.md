...
| 版本 | 日期 | 修改內容 | 修改人 |
|------|------|----------|--------|
| v1.8 | 2026/06/03 | 依議題 2026-05-23-promotion-batch-gift-limit-pagination §6.2F.1.D：批次操作補齊跨頁選取行為——兩段式選取（本頁全選 → 主動擴大至跨頁）、上限 500 筆、操作完成自動清除選取 | Una |
| v1.7 | 2026/06/02 | 依議題 2026-05-20-lp-guest-email-abandon-journey §8.5.2 §6.3B：新增 LP 訪客棄單 guest_email 路徑例外說明；§8.5.2「訪客不觸發」改為「一般電商訪客不觸發，LP 訪客走獨立 guest_email 路徑」；§6.3B 步驟 2a 加 LP 例外備註；cart_recovery_tokens.member_id 說明可 nullable | Una |
| v1.6 | 2026/05/28 | §8.5 全面移除 SQL 語法（CREATE TABLE / ALTER TABLE / pseudo-schema）；改以業務語言描述欄位定義與資料需求；§8.5.3 coupon_usages、§8.5.5 marketing_journey_logs、§8.5.8 cart_items、§8.5.9 退款原子性、§8.5.10 應收抵扣邏輯皆改寫；member_point_logs ALTER TABLE 改為欄位需求說明；工程師重點移除 Redis/SELECT FOR UPDATE/Laravel/LINE 速率數字等實作細節 | Una |
| v1.5 | 2026/05/28 | §6.2C 補齊缺漏建立欄位規格：新增「免運活動額外欄位」（含欄位顯示條件說明）、「折扣活動額外欄位」（支援多階梯 1–5 組，含結帳最高階梯套用規則）；對應 Prototype 核對清單三-5 缺口 | Una |
| v1.4 | 2026/05/27 | 四、行銷活動六項議題落地：4-1 Mode B 後端時程改為未定（移除 v1.3 承諾）；4-2 購物車挽回券改為一次性專屬券；4-3 加購品條件破壞確立永久不處理政策；4-4 旅程 Log 保留 365 天排程清理；4-5 應收帳款 Webhook 確認不提供（不列入任何版本）；4-6 LINE Webhook Phase 2 方向備忘（Bot + 客服轉接，互動選單維持待議題） | Una |
...
| 文件狀態 | **v1.8** — §6.2F.1.D 批次操作補齊跨頁選取行為：兩段式選取（本頁全選 → 主動擴大至跨頁）、上限 500 筆、操作完成自動清除選取 |
| 作者 | Una |
...
| 欄位 | 元件 | 顯示條件 | 說明 |
| --- | --- | --- | --- |
| 免運類型 | `<el-radio>` | 永遠顯示 | 整單免運 / 部分商品免運 |
| 條件類型 | `<el-radio>` | 免運類型 = 整單免運 | 滿額門檻（NT$X）/ 滿件門檻（N 件）|
| 條件數值 | `<el-input-number>` | 免運類型 = 整單免運 | 必填；滿額填金額（NT$）、滿件填件數 |
| 計算基準 | `<el-select>` | 免運類型 = 整單免運 AND 條件類型 = 滿額門檻 | 折扣後金額 / 購物金點數折抵後金額；旁加 ⓘ tooltip 說明（詳見 §6.2G.1）|

> 注意：「部分商品免運」無條件設定，免運對象由通用欄位「適用商品範圍」指定商品決定；選「部分商品免運」時條件類型 / 條件數值 / 計算基準三欄隱藏。§6.2G 有完整免運優先序規格。

**折扣活動額外欄位：**

支援多階梯設定（例：滿 NT$1,000 折 NT$100 / 滿 NT$2,000 折 NT$250）。每筆活動可設 1–5 個階梯，結帳時自動套用消費者符合的最高階梯。

| 欄位 | 元件 | 說明 |
| --- | --- | --- |
| 折扣條件類型 | `<el-radio>` | 滿額（NT$X）/ 滿件（N 件）；同一活動所有階梯共用同一條件類型 |
| 階梯設定（可新增最多 5 組）| 動態列表 `<el-button>` 新增列 | 每組包含：條件數值 + 折扣類型 + 折扣數值；條件數值需遞增排列，系統於儲存時驗證 |
| └ 條件數值 | `<el-input-number>` | 必填；同類型不可重複；需大於前一階梯 |
| └ 折扣類型 | `<el-radio>` | 百分比折扣 / 固定金額折扣；同一活動所有階梯共用同一折扣類型 |
| └ 折扣數值 | `<el-input-number>` | 必填；百分比填 1–99（整數）、固定金額填 NT$ 正整數；建議折扣力度隨階梯遞增（系統不強制但提示）|

> 結帳計算規則：套用消費者符合的最高階梯，不累加。例：設定滿 1,000 折 100 / 滿 2,000 折 250，消費 NT$2,500 → 套用 NT$250 折扣，不是 NT$350。僅需設一個門檻時，建立一組階梯即可。
...
活動列表頁支援跨頁累積選取後批次設定贈品上限，單次操作上限 500 筆。

**選取流程（兩段式，防止誤觸大量操作）：**

1. 商家勾選當前頁面活動，工具列顯示「已選取 N 筆」。
2. 頁面上方出現提示橫幅：「已選取本頁 N 筆。選取全部 X 筆活動」，商家需主動點擊才能把選取範圍擴大到所有頁面，系統不自動擴大。
3. 若累計選取筆數超過 500，系統提示「最多可同時操作 500 筆，請分批執行」，不允許繼續新增選取。

**批次設定彈窗（選取完成後點擊批次操作按鈕觸發）：**

- 選項一：改為使用共用上限（回歸全域設定值）
- 選項二：統一設定為指定值（輸入 1–15 的整數）

操作送出後，選取狀態自動清除。視覺標示（共用上限 vs. 單獨設定）依 §6.2F.1.C 規範維持不變。
...
> 依 PM 補充裁決：採商家自助開通；v1.2 完成開通設定 UI + audit log，**後端 Mode B 邏輯時程未定，視後續產品需求與工程評估再決定**。
>
...
#### C. 切換規則（v1.2 預留設計，後端時程未定）

| 情境 | 系統行為（後端實作時程未定，視產品需求再評估）|
|------|----------------|
...
| 2 | 加購品有達原始門檻 + 退原商品 | **系統不處理**：加購資格為下單當下一次性判斷，退原商品後加購品維持原訂單狀態，不強制退回。**永久政策，不列入後續版本迭代。** |
...
2. 對每個購物車：
   a. 確認 member_id 非空（訪客購物車不觸發本旅程；LP 訪客棄單另走 guest_email 路徑，見 §8.5.2 LP 例外說明）
   b. 確認該 member_id 在冷卻期內未觸發過此旅程（查 marketing_triggers 表）
   c. 確認購物車金額 >= 最低購物車金額設定
3. 符合條件者：
   a. 產生個人化挽回連結（含 cart recovery token，有效期 48 小時）
   b. 若有設定優惠券：為該會員動態建立一次性專屬券碼（非共用券）；券碼使用後立即失效，或於 48 小時後過期（以先到者為準）
   c. 呼叫 NT 模組發送 Email
...
> **決議 4-2：購物車挽回採一次性專屬券（非共用券）**：每次挽回旅程觸發時為該會員動態建立獨立券碼，券碼使用後立即失效，或於 48 小時後過期（以先到者為準）；儲存於 `exclusive_coupon_instances` 表（或擴充 `cart_recovery_tokens` 表），欄位至少包含 `member_id`、`coupon_template_id`、`unique_code`、`expires_at`、`used_at`。此設計確保行銷追蹤精準度：可逐筆追溯哪張券碼促成了回購。
...
> **4-6 LINE Webhook Phase 2 方向備忘（非正式規格）：**
> - **v1.0 範圍**（不變）：僅處理「加入好友」與「取消追蹤」兩個 Webhook 事件
> - **Phase 2 方向（若排程允許）**：Bot 自動回覆（常見問題/訂單查詢）、客服轉接（消費者傳訊 → 後台收件匣）；上述為次要功能，非主要規格內容
> - **互動式選單（Rich Menu）**：範圍未定，維持待議題狀態，不列入任何階段規格
...
> v1.2 修訂：依 2026-05-21 主會議議題 5 + 補充會議議題 2 決議，改為**雙模式**規格（Mode A 預設、Mode B 後端時程未定）。

**雙模式對照：**

| 情境 | Mode A（v1.2 預設）| Mode B（v1.2 開通介面已具備、後端時程未定）|
|------|-------------------|----------------------------------|
| 全額退款 | 全額追回 | 同 Mode A |
...
系統需記錄每筆應收回饋的追回狀態，供後續自動抵扣與到期註銷使用。需記錄以下資訊：

| 欄位 | 業務說明 |
|------|---------|
| 會員 | 應收對象 |
| 應收金額 | 正數；單位依類型為點數或購物金 |
| 類型 | 兩種值：`points`（點數）/ `wallet`（購物金）|
| 來源訂單 | 觸發此應收記錄的原始訂單 |
| 狀態 | 三種值：`pending`（待追回）/ `recovered`（已抵扣）/ `voided`（已註銷）|
| 到期時間 | 應收產生日起算 6 個月後到期 |
...
點數異動紀錄（`member_point_logs`）需能識別每筆異動的來源類型與關聯對象，以支援對帳與追蹤：

| 欄位 | 業務說明 |
|------|---------|
| 來源類型 | 四種值：order（訂單回饋）/ refund（退款追回）/ manual（手動調整）/ expiry（到期註銷）|
| 關聯對象 | 對應的訂單 ID 或其他觸發來源；手動調整時可為空 |

退款追回的異動金額為負數，來源類型標記為 `refund`。
...
#### 6.6E 應收帳款 Webhook 事件（確認不提供）

> **4-5 決議：確認不提供 `receivable.voided` Webhook 事件，且不列入任何版本規劃。**
>
> 決策原因：Webhook 設定 UI 約需 5 人日工時，v1.0 效益評估不符；商家如需監控應收帳款作廢，請使用後台應收餘額對帳報表（§6.6C）主動查詢。
...
| 7 | 回饋活動訂單取消／部分退款 | §6.6B | Mode A 應收記錄、Mode B 負餘額（後端時程未定）|
| 8 | A+B 組合件數計入滿件門檻 | §6.2D | 計入全店滿件門檻 |
| 9 | 加購品退款解除原折扣 | §6.2J | 不重算 + 揭露文案 |
| 10 | 多組贈品超過軟上限 10 | §6.2F | 後端 log warning；超過硬上限 15 前端阻擋 |
| 11 | 加購品有達原始門檻 + 退原商品 | §6.2J #2 | **系統不處理（永久政策）**：加購資格為下單當下一次性判斷，不隨後續退貨連動，加購品維持原訂單狀態 |
...
**一般電商（購物車模組）：** 挽回旅程**僅針對已登入會員的購物車**，訪客（Cookie/Session 購物車）不觸發本旅程。

訪客在同一 Session 中登入後，購物車合併為會員購物車，**合併後的會員購物車**才納入挽回監控（計時從合併時重新開始）。

**【LP 例外】一頁式商店訪客棄單旅程（依議題 2026-05-20-lp-guest-email-abandon-journey 決議）：**

LP（一頁式商店）訪客在結帳頁成功提交 Email 後棄單，走**獨立的 guest_email 路徑**，不走本章節的 member 挽回旅程。主要差異：

| 項目 | 一般購物車挽回（member 路徑）| LP 訪客棄單挽回（guest_email 路徑）|
| --- | --- | --- |
| 識別依據 | `member_id` | `guest_email` |
| 冷卻期去重 | `marketing_triggers.member_id` | `marketing_triggers.guest_email` |
| 優惠券 | 支援一次性專屬券（決議 4-2）| 本期不支援 |
| LINE 發送 | 有 LINE 綁定時觸發 | 不觸發（訪客無 LINE 綁定）|
| 發送時機 | 後台設定（通用 N 小時）| 固定 1 小時（LP 獨立設定）|
| 合規同意 | 會員注冊時已同意 | 結帳頁 checkbox 即時取得（需記錄同意版本）|

LP 訪客棄單挽回完整規格見一頁式商店 PRD §6.9C。`cart_recovery_tokens` 表的 `member_id` 欄位在 guest 路徑下可為空，改以 `guest_email` 識別。

### 8.5.3 優惠券使用的 Race Condition 防護

使用 DB 唯一索引保證冪等性：

優惠券使用紀錄（`coupon_usages`）以「同一會員同一張券只能有一筆使用紀錄」為唯一性約束，確保冪等：
...
結帳時若同一會員對同一張券重複提交，系統回傳「優惠券已使用（409）」。全域使用次數限制在資料庫層以原子方式遞增，防止並發超用。
...
行銷旅程觸發紀錄（`marketing_journey_logs`）用於冷卻期去重與效益追蹤：

| 欄位 | 業務說明 |
|------|---------|
| 會員 | 被觸發的對象 |
| 旅程類型 | 六種值：cart_recovery（購物車挽回）/ sleep_wakeup（沉睡喚醒）/ churn_winback（流失挽留）/ post_purchase（購後推薦）/ points_expiry（點數到期）/ coupon_expiry（優惠券到期）|
| 觸發時間 | 旅程實際觸發的時間點 |
| 冷卻結束時間 | 此時間前同一旅程不再對該會員觸發 |
| 發送管道 | email / line / both |
| 關聯對象 | 對應的購物車 ID 或訂單 ID；依旅程類型而定 |
| 執行結果 | 三種值：sent（已發送）/ failed（發送失敗）/ skipped（條件未達跳過）|

> **Log 保留政策（4-4 決議）：** 行銷旅程紀錄保留 **365 天**，足以支撐年度行銷效益分析；超過一年的歷史 log 實務使用率低，不納入長期儲存成本。由後端排程每日清理過期記錄。若未來有合規要求，再行調整保留期限。

去重判斷：查詢同一會員、同一旅程類型、冷卻結束時間仍未到期的紀錄；若存在則跳過。
...
贈品以獨立行項存入購物車，與一般商品分開計算。購物車商品行項需能識別以下資訊：

| 欄位 | 業務說明 |
|------|---------|
| 商品 | 對應的商品 ID |
| 數量 | 件數 |
| 單價 | 結帳時金額；贈品為 0 元，不計入訂單金額 |
| 是否贈品 | 布林值；贈品行項不計入任何折扣門檻與金額 |
| 來源贈品活動 | 觸發此贈品的行銷活動 ID；用於追蹤與失效判斷 |

贈品庫存不足時：依活動設定處理，並於行項標記「庫存不足」，前台顯示「此贈品庫存不足，已自動移除」。

### 8.5.9 回饋活動與訂單退款的原子性保護

回饋追回（§6.6B）需與退款動作**原子性同步**，確保退款金額和回饋扣回不會發生部分成功的不一致狀態。執行邏輯如下：

1. 呼叫金流退款
2. 退款成功後，依退款比例計算應追回的點數或購物金
3. 寫入點數異動紀錄（負數金額，來源類型標記為退款）
4. 更新會員點數餘額與購物金餘額
5. 若餘額不足以抵扣：寫入應收記錄（Mode A）；Mode B 後端時程未定
6. 任一步驟失敗則全部回滾，不產生部分扣回的狀態

### 8.5.10 應收記錄抵扣的 Transaction 保護（v1.2 新增）

新回饋發放時，若該會員存在未結清的應收記錄，需先抵扣應收，剩餘部分才入帳。抵扣邏輯如下：

1. 計算會員所有「待追回」應收記錄的合計金額 R
2. 本次新回饋金額 N：
   - 若 N ≥ R：全數清償應收紀錄（狀態 → 已抵扣），剩餘 (N - R) 正常入帳
   - 若 N < R：依「先進先出」原則優先清償最早的應收紀錄，直到 N 用盡
3. 寫入點數異動紀錄或購物金異動紀錄
4. 整個流程需原子性完成，任一步驟失敗則全部回滾
...
1. 購物車挽回排程每 5 分鐘執行一次，需防止同一批次被重複觸發（建議加分散式鎖確保排程不重疊）
2. LINE 推播批次量大時需分批發送，避免超過 API 速率限制
3. Cart Recovery Token 需防並發重複使用（同一 Token 被雙擊的情境）；實作需確保同一 Token 只能被核銷一次
4. 優惠套用的計算邏輯（§7.1 流程圖）必須完整實作優先序，不可自行調整順序
5. LINE Channel Access Token 屬敏感憑證，需加密儲存，禁止在 Log 或前端中明文出現
6. **v1.2 新增**：應收記錄的抵扣需在新回饋發放的原子操作內完成（§8.5.10）
7. **v1.2 新增**：`negative_balance_allowed` 設定變更需 audit log；v1.2 只記錄不執行 Mode B 邏輯（§8.5.12）；**Mode B 後端時程未定，視後續產品需求與工程評估再決定**
8. **v1.2 新增**：贈品上限「共用 vs 單獨設定」需於前端動態判斷視覺差異
9. **v1.2 新增**：加購品分區需在購物車頁與訂單詳情頁兩個前端介面同步實作
10. **v1.2 新增**：活動衝突欄位（`conflict_with_activity_ids`）由後端排程或儲存事件計算更新，前端不直接寫入
