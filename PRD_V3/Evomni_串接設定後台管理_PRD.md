...
| 版本 | 日期 | 修改內容 | 修改人 |
|------|------|----------|--------|
| v1.0 | 2026/04/28 | 初稿建立：串接設定中心 Hub、全自動/設定即用/半人工/人工處理廠商設定頁、Webhook 管理完整規格 | Una |
| v1.6 | 2026/06/01 | 依議題 2026-05-20-integration-shop-domain-source §6.2 §8.2：修正決議 24-4 描述——`shop_domain` 由系統指派（網域上線後回傳寫入，唯讀），現階段不支援自訂網域；移除「自訂網域優先，fallback 子網域」邏輯（該描述為未來擴充項目，非當前現況） | Una |
| v1.5 | 2026/06/01 | §6.2B 區塊五新增「兩者皆失敗」時的連線測試持久狀態顯示規格：進入詳情頁時若 last_test_result='failed' 且 Webhook 連續失敗，顯示 warning Alert（含錯誤代碼 / 時間 / 立即重新測試按鈕），成功測試後自動消除 | Una |
| v1.4 | 2026/06/01 | 依議題 2026-05-20-integration-error-badge-source §6.1：Badge 判定改採方案 (c)，兩者任一失敗即顯示 Badge；文案依失敗原因區分（連線測試失敗 / 串接異常），兩者皆失敗時顯示「串接異常」；撤銷條件為兩者皆正常；Hover Tooltip 動態顯示當前失敗原因（取代 v1.3 方案 b 規格） | Una |
| v1.3 | 2026/06/01 | 依議題 2026-05-20-integration-error-badge-source §6.1：「連線異常」Badge 改為僅依 Webhook 連續失敗判定（預設閾值 3），移除連線測試失敗觸發（已由 v1.4 取代） | Una |
| v1.2 | 2026/05/28 | 24-2：§6.5 SKU 對照 CSV 批次匯入新增「合併/全替換」模式選擇 Dialog；24-3：§6.7 Webhook 失敗通知確認 24h 去重視窗設計（同廠商24h內不重複寄送）；24-4：§6.2、§8.2 Webhook URL 的 shop_domain 確認從 `system_settings.shop_domain` 取值 | Una |
| v1.1 | 2026/05/27 | 依 §六串接設定決議：新增待討論標記 6-5 連線異常 Badge 判定邏輯 | Una |

---

# Evomni — 串接設定後台管理介面 產品需求文件 (PRD) v1.1
...
| 文件狀態 | v1.5 — §6.2B 區塊五補齊「兩者皆失敗」時連線測試持久狀態顯示規格 |
...
| 連線測試失敗 | `#F56C6C`（紅） | 連線測試失敗 | 連線測試失敗，且 Webhook 狀態正常 |
| 串接異常 | `#F56C6C`（紅） | 串接異常 | Webhook 連續失敗達門檻（預設 3 次），或連線測試與 Webhook 兩者皆失敗 |

> **連線異常 Badge 判定邏輯（依議題 2026-05-20-integration-error-badge-source 決議，採方案 c）：**
>
> 兩者任一失敗即顯示紅色 Badge，但文案依失敗原因區分（商家技術能力不一，寧可多提示、不要漏掉問題）：
>
> | 觸發原因 | Badge 文案 | Tooltip 說明 |
> | --- | --- | --- |
> | 僅連線測試失敗（Webhook 正常）| 連線測試失敗 | 上次連線測試未通過，建議重新測試或檢查金鑰設定 |
> | 僅 Webhook 連續失敗（連線測試正常）| 串接異常 | Webhook 連續 3 次未回應，可能影響訂單通知，請檢查串接設定 |
> | 兩者皆失敗 | 串接異常 | 顯示 Webhook 失敗說明；連線測試失敗另於廠商詳情頁分區塊說明 |
>
> **Badge 撤銷條件：** 兩項狀態皆正常才撤銷。商家執行修復後系統重新判定，任一仍失敗則維持 Badge，Tooltip 更新為當前實際失敗原因（避免商家修復 Webhook 後誤以為全部修好，但連線測試仍失敗未處理）。
>
> **Webhook 連續失敗閾值：** 預設 3（`consecutive_failure_threshold`），後續若客服反映誤報率高再調整。
...
- **連線測試失敗 / 串接異常 Card**：Card 頂部顯示紅色細線（4px 高），懸停（Hover）時 Tooltip 依當前失敗原因動態顯示（見上方 Badge 判定邏輯表）
...
- URL 格式：`https://[商家網域]/api/webhooks/payment/[uuid]/[廠商代號]`
  - **`商家網域` 來源（決議 24-4，依議題 2026-05-20-integration-shop-domain-source 修正）：** 從 `system_settings.shop_domain` 取值。此欄位由系統於網域上線後回傳並寫入，商家無法自行設定（唯讀）。現階段不支援自訂網域；自訂網域覆蓋邏輯為未來擴充項目，屆時另行補充規格。

**區塊五：連線測試**

**（A）持久狀態顯示——進入詳情頁時**

若廠商 Badge 為「串接異常」（Webhook 連續失敗）且 `last_test_result = 'failed'`，進入詳情頁時在「測試連線」按鈕上方顯示持久 Alert，讓商家清楚知道連線測試也有問題尚待處理：

```
<el-alert type="warning" :closable="false">
  ⚠️ 上次連線測試未通過
  錯誤代碼：[code] — [說明]　　測試時間：[timestamp]
  建議確認金鑰是否正確後重新測試。
  [立即重新測試]（inline primary button）
</el-alert>
```

此 Alert 在商家執行新一次測試並成功後自動消失（`last_test_result` 更新為 `success`）。

**（B）即時測試互動**

- 按鈕：`測試連線`（`<el-button type="default" class="!rounded-none">`）
- 點擊後：Loading 狀態，按鈕文字變「測試中…」
- 成功結果：`<el-alert type="success">` 顯示：`連線成功！廠商 API 回應正常，憑證驗證通過。`（持久狀態 Alert 同步消除）
...
- 操作按鈕：
  ...
  - **匯入模式選擇（決議 24-2）：** 點擊「批次匯入」後彈出 Dialog，商家選擇匯入模式：
    - **○ 合併（Upsert）**：CSV 以外的既有對照保留；僅更新 CSV 中出現的 SKU
    - **○ 全部替換**：先清除此廠商所有現有對照，再插入 CSV 內容（操作前顯示紅色警示：「此操作將清除目前所有 XXX 的 SKU 對照，確定繼續？」）
  - `下載對照表`（匯出目前設定為 CSV）
...
- **通知去重視窗（決議 24-3）：** 同一廠商 **24 小時內不重複寄送失敗通知 Email**（確認現行設計）；24 小時計算從同廠商上一次成功發送通知信起算；廠商 Webhook 恢復正常後，下次失敗重新計數
...
| Webhook URL 唯一性 | 每家廠商的 Webhook URL 包含 UUID token（`/api/webhooks/payment/{uuid}/{vendor}`），避免 URL 枚舉；`{host}` 從 `system_settings.shop_domain` 取值（決議 24-4）；此值由系統指派、唯讀，現階段不支援自訂網域 |
