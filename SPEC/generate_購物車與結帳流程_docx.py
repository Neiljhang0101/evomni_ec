"""Convert 購物車與結帳流程 UX Spec markdown to DOCX for Claude Design upload."""
import re
import os
...
def process_md_to_docx(md_path, out_path):
    ...
    run = title_para.add_run("購物車與結帳流程")
    run.bold = True
    run.font.size = Pt(22)
    ...
    run1 = sub1.add_run("Design Brief + UX Spec")
    run1.font.size = Pt(14)
    run1.font.name = "Noto Sans TC"
    run1.font.color.rgb = RGBColor(0x2B, 0x5E, 0xA7)

    sub2 = doc.add_paragraph()
    sub2.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run2 = sub2.add_run("Version: 1.0  |  Created: 2026-06-02  |  用途：上傳至 Claude Design，配合 Prompt Cheatsheet 使用")
    ...
    while i < len(lines):
        ...
        # Skip the first H1 (already in cover) and version metadata block
        if i == 0 and stripped.startswith("# "):
            i += 1
            continue
        if stripped.startswith("> **Version:**") or stripped.startswith("> **Created:**") or \
           stripped.startswith("> **PRD") or stripped.startswith("> **適用") or \
           stripped.startswith("> **用途"):
            i += 1
            continue
...
if __name__ == "__main__":
    ...
    md = os.path.join(base, "購物車與結帳流程-UX-Spec.md")
    out = os.path.join(base, "購物車與結帳流程-UX-Spec.docx")
