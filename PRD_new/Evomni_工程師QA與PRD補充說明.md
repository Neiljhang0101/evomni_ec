...
- **PM 回覆（規格決策）**：
  ...
  - **商品**：軟刪除（`is_deleted = true`），已關聯的訂單繼續顯示商品名稱快照（訂單建立時快照商品名稱、規格、單價至 `order_items` 表）
...
- **工程師問題**：`退貨商品小計 × 折扣分攤率` 可能產生小數，例如折扣 10 元、退 1 件共 3 件，分攤為 3.33...元。最終退款金額要用 FLOOR、ROUND 還是 CEIL？金流廠商退款 API 接受小數嗎？
...
- **PM 回覆（規格決策）**：
  ...
  - 特殊規則：若多件同商品退貨，先逐件計算 `FLOOR`，不可先加總再 `FLOOR`（避免累積誤差偏大）
...
- **PM 回覆（規格決策）**：
  - 每次退款的折扣分攤**永遠以原始訂單的商品小計總額為分母**
  - 計算公式：`退貨折扣分攤 = 本次退貨商品小計 × (原始訂單總折扣 ÷ 原始訂單商品小計)`
...
- **PM 回覆（規格決策）**：
  ...
  - 扣回公式：`total_spent -= 退款金額（不含運費）`（只扣商品退款部分，不扣可退運費）
...
- **工程師問題**：消費者加入商品 A（11:00），等了 1 小時又加商品 B（12:00）。購物車挽回的「1 小時計時」是從 11:00 還是 12:00 算起？

- **PM 回覆（規格決策）**：
  - 計時從**最後一次購物車異動時間**起算（`carts.last_updated_at`）
  - 每次購物車有任何變動（新增/刪除商品、修改數量），重置計時
  - 這樣設計的原因：消費者正在猶豫中的購物車才觸發，而不是還在添加商品的購物車
...
- **工程師問題**：Layer 2 的滿額折扣，計算基準是商品原始小計還是 Layer 1 折後金額？這影響計算結果很大。
...
- **工程師問題**：同一商品在一頁式商店和主商店同時銷售，庫存共用嗎？如果兩邊同時下單，有超賣風險嗎？
...
**Q-2.01｜商品重量的預設值：如果商家沒有填商品重量欄位**

- **工程師問題**：重量費率以商品重量為基準，但如果商家沒有填商品重量，系統要怎麼計算？

- **PM 回覆（規格決策）**：
  - 商品中心（Part 2）`products` 表必須新增 `weight DECIMAL(8,3)` 欄位（單位：公斤，預設值 `NULL`）
  - 若使用「重量費率」計算，但商品 `weight = NULL`：
    - **結帳頁顯示警告**：`此訂單包含未設定重量的商品，運費以最低費率計算，請聯繫商家確認`
    - 後端以 `weight = 0` 計算（即計入最低重量區間）
  - 商家後台商品設定頁：若啟用了重量計費，商品重量欄位顯示 **`(必填)` 提示**（非強制驗證，不阻止儲存，但顯示醒目提示）
...
- **PM 回覆（規格決策）**：
  ...
  - 購物車頁若有混合溫層商品，顯示 `<el-alert>` 溫層警示（「您的購物車包含不同溫層商品，結帳時將依最高溫層收取運費」）—— **但不顯示具體金額**
