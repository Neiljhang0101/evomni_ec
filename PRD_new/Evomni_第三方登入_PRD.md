# Evomni - 第三方登入（LINE / Google / Facebook）產品需求文件 (PRD) v1.0
...
| 文件狀態 | **P1 首次完整規格** — 涵蓋 LINE / Google / Facebook OAuth 2.0 消費者登入、帳號綁定/解綁、後台憑證設定、帳號合併安全流程 |
| 作者 | 廖紫茵（Claude 依授權產出）|
...
- LINE 登入在 LINE App 內瀏覽器（LIFF）時的特殊處理：使用 LINE LIFF SDK，如不在 LIFF 環境則走標準 OAuth
...
CREATE TABLE member_social_accounts (
    ...
    platform        ENUM('line', 'google', 'facebook') NOT NULL,
    platform_uid    VARCHAR(255) NOT NULL,           -- 第三方平台的使用者 ID
    platform_name   VARCHAR(100),                    -- 第三方平台的顯示名稱
    platform_email  VARCHAR(255),                    -- 第三方平台提供的 Email（可能與帳號 Email 不同）
    linked_at       DATETIME DEFAULT CURRENT_TIMESTAMP,
    last_login_at   DATETIME NULL,
    created_at      DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (member_id) REFERENCES members(id),
    UNIQUE KEY uniq_platform_uid (platform, platform_uid),  -- 同平台同 UID 只能綁一個帳號
    INDEX idx_member_id (member_id)
);

-- OAuth 臨時 State 表（也可用 Redis，TTL 10分鐘）
CREATE TABLE oauth_states (
    state       CHAR(36) PRIMARY KEY,   -- UUID
    platform    ENUM('line', 'google', 'facebook') NOT NULL,
...
-- 帳號安全事件 Log
CREATE TABLE member_security_logs (
    id          BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    member_id   BIGINT UNSIGNED NOT NULL,
    event_type  ENUM('social_linked', 'social_unlinked', 'password_changed',
                     'login_failed', 'account_locked') NOT NULL,
    detail      JSON,    -- {"platform": "line", "ip": "1.2.3.4"}
    ip_address  VARCHAR(45),
    created_at  DATETIME DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_member_id_event (member_id, event_type)
);
```
...
- **目前有 1 個待確認事項**：LINE LIFF（LINE App 內建瀏覽器）的特殊情境是否需要支援；若客群以 LINE 結帳為主，建議納入；需和前端評估 LIFF SDK 的導入成本
