...
| 文件狀態 | v1.7 — 新增 §6.12 通知信管理、§6.13 會員設定（全局開關），補齊原型空白頁規格 |
| 作者 | Una |
...
| 產品追蹤清單 | 心願清單 | 進階電商包 | 追蹤感興趣商品 | 消費者可收藏商品至追蹤清單；商家後台可查看哪些商品被收藏最多次 | 消費者、商家管理員 |
...
| 品類愛好者 | 同一商品分類完成訂單次數 ≥ 3 次；自動偵測購買頻次最高的分類，並記錄分類名稱 | `#409EFF`（藍） | Part 4 優惠券發放可篩選此標籤 |
...
**對應 DB：**
```sql
-- system_settings 表新增（若尚未存在）
INSERT INTO system_settings (key, value) VALUES ('negative_balance_allowed', 'false');

-- audit log 寫入規範
INSERT INTO system_settings_audit_logs
  (setting_key, old_value, new_value, operator_id, created_at)
  VALUES ('negative_balance_allowed', 'false', 'true', ?, NOW());
```
