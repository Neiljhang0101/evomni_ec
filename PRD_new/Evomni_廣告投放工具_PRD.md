...
| 商品目錄 Feed | Facebook Catalog Feed | 提供 Facebook 動態商品廣告所需的商品目錄 | 系統自動生成符合 Facebook Product Catalog 規格的 CSV Feed；Feed URL 格式：`{store_domain}/feeds/facebook-catalog.csv`；包含欄位：id, title, description, availability, condition, price, sale_price, image_link, link, brand, product_type, google_product_category | 商家管理員 |
| 商品目錄 Feed | Google Merchant Center Feed | 提供 Google 購物廣告所需的商品目錄 | 系統自動生成符合 Google Merchant Center 規格的 XML Feed（RSS 2.0 格式）；Feed URL：`{store_domain}/feeds/google-merchant.xml`；包含欄位：g:id, g:title, g:description, g:link, g:image_link, g:availability, g:price, g:sale_price, g:condition, g:brand, g:product_type | 商家管理員 |
...
-- 本功能僅使用以下欄位：
-- integration_settings.feed_token       CHAR(36) NOT NULL   -- UUID，Feed URL 保護 token
...
- 這次的規格是關於**廣告投放工具 v2.0**，定位為「廣告追蹤碼設定完成後的廣告操作層」，聚焦三大工具：UTM 網址生成器 / 商品目錄 Feed / 廣告來源分析。廣告追蹤碼（FB Pixel / GA4 / GTM）已由「行銷管理 > 行銷工具設定」統一管理，本模組不重複處理。
- **工程師注意**：
  1. 廣告追蹤碼相關欄位（pixel_facebook / ga4_id 等）**不需要新增**，沿用行銷工具設定既有的 `integration_settings` 欄位；本模組僅新增 `feed_token` 欄位（若尚未存在）
  2. 商品 Feed 生成要走 Queue Job 避免 Timeout，1000 件商品大約需要 10-30 秒
