...
| 日期 | 2026/05/22 |
| 作者 | Una |
| 文件狀態 | v1.1 — CC-1～CC-5 全數定案：訪客結帳後流程完整規格 / 購物車過期設定 / 一頁式商店共用結帳流程 / 0 元商品強制登入 / LP 優惠套用規則（含商業情境說明） |
...
> 本節為 **A3 議題定案**，提供跨模組共用的訂單狀態定義。所有涉及訂單狀態判斷的邏輯（商品刪除阻擋、訂單列表計數、報表邊界）均以本節為依據。
...
completed ──全額退款完成──→ closed

（任何進行中狀態申請全額退款，退款完成後進入 closed）
```

> ⚠️ 已出貨（`shipped`）及之後的狀態不得直接取消；需先走退換貨申請流程，退款完成後才進入 `closed`。
...
| 判斷方式 | SQL 邏輯 |
|----------|----------|
| 進行中 | `status NOT IN ('completed', 'cancelled', 'closed')` |
| 最終態（非進行中）| `status IN ('completed', 'cancelled', 'closed')` |

**三個使用場景的套用：**

| 場景 | 邏輯說明 |
|------|----------|
| **商品刪除阻擋** | 若該商品任一 SKU 存在於進行中訂單的 `order_items` 中 → 禁止刪除，顯示阻擋提示 |
| **後台訂單列表「進行中」Tab 計數** | `SELECT COUNT(*) FROM orders WHERE status NOT IN ('completed', 'cancelled', 'closed')` |
...
- 當 `SUM(order_refunds.amount) = orders.total` 時，系統自動將訂單狀態轉為 `closed`
...
-- 3. 扣減庫存
UPDATE product_variants SET stock = stock - order_quantity WHERE id = ?;
...
| 付款方式 | `orders.expires_at`（從訂單建立起算）|
|----------|--------------------------------------|
| 信用卡 / LINE Pay | 30 分鐘 |
| ATM 虛擬帳號 / 超商代碼 | 72 小時 |
| 貨到付款 | `NULL`（永不逾時）|

> 30 分鐘預設值可於後台「全域設定 > 電商進階設定 > 購物車逾時」調整（見[全域設定：電商設定 PRD](Evomni_全域設定_電商設定_PRD.md)）
...
| `triggered_by` | ENUM | `return_order`（退換貨）/ `manual`（手動）|
