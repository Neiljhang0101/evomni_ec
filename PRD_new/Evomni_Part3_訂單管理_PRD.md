...
| v2.1 | 2026/05/29 | 合併補齊：§6.8 美安訂單資訊卡、§6.9 訂單設定頁、§6.10 Toast 訊息完整清單、§9 原型連結；更正訂單狀態顏色（已出貨→藍 #409EFF、配送中→淡藍 info、已完成→灰 #909399） | Neil（Claude 依授權產出）|

# Evomni — 訂單管理 產品需求文件 (PRD) v2.1
...
| 版本 | v2.1 |
| 日期 | 2026/05/29 |
| 需求來源 | Master PRD v1.0 Chapter 5（P0）、PRD V3 §3.2.6、方案規格 V1.1、**Part4 行銷活動 PRD v1.2 連動需求**、Prototype `html/訂單管理.html` |
| 文件狀態 | v2.1 — 合併 v1.2（Part4 連動）與 v2.0（Prototype 對齊），補齊美安訂單資訊卡、訂單設定頁、Toast 清單；更正狀態顏色 |
| 作者 | Una / Neil（Claude 依授權產出）|
| 對應方案 | 電商啟航方案 ✅ / 進階電商包 ✅ |
...
| 付款方式 | 100px | 文字顯示 |
| 發票狀態 | 120px | `<el-tag class="!rounded-full">`；「未開立」且貨到付款已完成者，標籤顏色 `#F56C6C`（紅），加 icon |
| 操作 | 140px | `<el-button size="small">` 查看詳情、`<el-button size="small" type="danger">` 取消（僅限「待付款」狀態） |

**訂單狀態 Tag 顏色對應（v2.1 更正版）：**
...
> 訂單共 8 種狀態，符合 §15「同一頁面狀態 ≥ 3 種」條件，所有狀態標籤旁均需加 ⓘ icon（14px，`#909399`），hover 顯示三層 tooltip。
...
當訂單 `is_meian = true` 時，訂單詳情頁左欄顯示「美安訂單資訊卡」。卡片內容依 `meian_status` 欄位動態切換。

#### 美安傳送狀態定義

| `meian_status` | 卡片狀態標籤 | 標籤顏色語意 |
| --- | --- | --- |
| `success` | 已傳送 | success（綠色）|
| `failed` | 傳送失敗 | danger（紅色）|
| `pending` | 待傳送 | warning（橙色）|
| `cancel_pending` | 取消通知待傳送 | warning（橙色）|
| `cancel_sent` | 取消通知已傳送 | neutral（灰色）|
...
```
訂單 {訂單編號} 已於 {YYYY-MM-DD HH:MM} 傳送至美安系統
確認碼：{美安確認碼}
```
...
按鈕：「重新傳送」（danger 樣式）→ 開啟確認對話框（見下方）。

**待傳送（pending）**

```
訂單尚未傳送至美安系統，預計於系統下一次排程時自動傳送。
```

按鈕：「立即手動傳送」（primary 樣式）→ 開啟確認對話框（見下方）。

**取消通知待傳送（cancel_pending）**

```
此訂單已取消，取消通知尚未傳送至美安系統。
```

按鈕：「傳送取消通知」（warning 樣式）→ 開啟確認對話框（見下方）。

**取消通知已傳送（cancel_sent）**

```
取消通知已於 {YYYY-MM-DD HH:MM} 傳送至美安系統
確認碼：{美安確認碼}
```

無操作按鈕。

#### 手動傳送 / 重新傳送確認對話框（雙重確認防呆）

- 標題：「確認重新傳送」或「確認手動傳送」
- 說明：「確定重新傳送訂單 {訂單編號} 至美安系統？」或「確定立即手動傳送訂單 {訂單編號} 至美安系統？」
- **安全確認輸入框**：「請輸入『確認』以解鎖確認按鈕」
  - 使用者需輸入文字「確認」，確認按鈕才從 disabled 變為 enabled
  - 此機制防止誤觸重複傳送
- 按鈕：「確認重新傳送」/ 「確認手動傳送」（primary，預設 disabled）、「取消」

成功後 Toast：「已重新傳送至美安系統」或「已手動傳送至美安系統」（success）

#### 取消通知確認對話框

- 標題：「確認傳送取消通知」
- 說明：「確定將訂單 {訂單編號} 的取消通知傳送至美安系統？此操作無法復原。」
- 按鈕：「確認傳送」（primary）、「稍後再傳」

成功後 Toast：「取消通知已傳送至美安系統」（success）

#### 美安報表匯出

訂單列表頁右上角「匯出美安報表」按鈕，開啟對話框：

- 欄位：日期範圍選擇器（開始日期 / 結束日期）
- 動態提示：「符合範圍的美安訂單：{N} 筆」
- 按鈕：「確認匯出」（primary）、「取消」
- 確認後 Toast：「美安報表產生中，完成後將寄送至您的信箱」（success）

#### 美安相關 DB 欄位補充建議

```sql
-- orders 表新增欄位
ALTER TABLE orders ADD COLUMN is_meian BOOLEAN DEFAULT FALSE;
ALTER TABLE orders ADD COLUMN meian_status ENUM('pending','success','failed','cancel_pending','cancel_sent') NULL;
ALTER TABLE orders ADD COLUMN meian_sent_at TIMESTAMP NULL;
ALTER TABLE orders ADD COLUMN meian_confirm_code VARCHAR(64) NULL;
ALTER TABLE orders ADD COLUMN meian_error_message TEXT NULL;
```
...
| 方法 | 路徑 | 說明 |
| --- | --- | --- |
| POST | `/api/orders/{id}/meian/send` | 手動傳送或重新傳送至美安 |
| POST | `/api/orders/{id}/meian/cancel-notify` | 傳送取消通知至美安 |
| GET | `/api/orders/meian/export` | 匯出美安報表（帶日期範圍參數）|

---

### 6.9 訂單設定頁

**導覽路徑：** 後台側邊選單 > 訂單管理 > 訂單設定（`order-settings`）

訂單設定頁管理付款逾時相關全局設定，僅超級管理員可操作。
...
| --- | --- |
| 欄位標籤 | 信用卡 / 行動支付逾時（分鐘）|
| 說明文字 | 超過設定時間仍未付款，系統將自動取消訂單並回補庫存。貨到付款不受此影響。|
| 元件 | 數字輸入器（±按鈕，上限 1440，下限 1）|
| 預設推薦值 | 30 分鐘 |
| 對應欄位 | `settings.order_payment_timeout_card`（單位：分鐘）|
...
| --- | --- |
| 欄位標籤 | 銀行轉帳逾時（小時）|
| 說明文字 | （同「信用卡」說明文字，強調貨到付款不受影響）|
| 元件 | 數字輸入器（±按鈕，上限 168，下限 1）|
| 預設推薦值 | 72 小時 |
| 對應欄位 | `settings.order_payment_timeout_atm`（單位：小時）|

> 注意：本頁設定與 §8.5.1 的技術規格對應。`expires_at` 計算邏輯由後端依這兩個設定值動態計算。

#### 操作按鈕

| 按鈕 | 行為 |
| --- | --- |
| 恢復預設值 | 將所有欄位重設為推薦值（30 分鐘 / 72 小時），不自動儲存 |
| 儲存設定 | 呼叫 `PATCH /api/settings/order`，成功後 Toast：「設定已儲存，新設定將套用於之後建立的訂單」|

#### 訂單設定 API 建議端點

| 方法 | 路徑 | 說明 |
| --- | --- | --- |
| GET | `/api/settings/order` | 取得訂單設定 |
| PATCH | `/api/settings/order` | 更新訂單設定 |

---

### 6.10 Toast 訊息完整清單（依 Prototype 對齊）

> 本節為各畫面所有 Toast 訊息的統一對照表，供前端工程師與 QA 參考。

#### 訂單列表（order-list）

| 觸發操作 | Toast 文字 | 類型 |
| --- | --- | --- |
| 點擊「批次列印出貨單」| 批次列印功能開發中 | info |
| 點擊「批次匯出」| 報表產生中，完成後將寄送至您的信箱 | info |
| 批次取消訂單成功 | 訂單已取消，庫存已自動回補 | success |
| 點擊「匯出」（一般）| 報表產生中，完成後將寄送至您的信箱 | info |
| 點擊「匯出美安報表」確認 | 美安報表產生中，完成後將寄送至您的信箱 | success |

#### 訂單詳情（order-detail）

| 觸發操作 | Toast 文字 | 類型 |
| --- | --- | --- |
| 更新訂單狀態成功 | 訂單狀態已更新，系統通知信已發送 | success |
| 開立發票成功 | 發票開立成功，發票號碼：{發票號碼} | success |
| 列印出貨單 | 列印功能開發中 | info |
| 儲存備註成功 | 備註已儲存 | success |
| 取消訂單成功 | 訂單已取消，庫存已自動回補 | success |
| 美安重新傳送成功 | 已重新傳送至美安系統 | success |
| 美安手動傳送成功 | 已手動傳送至美安系統 | success |
| 美安取消通知成功 | 取消通知已傳送至美安系統 | success |

#### 退換貨列表（rma-list）— 快速審核

| 觸發操作 | Toast 文字 | 類型 |
| --- | --- | --- |
| 快速審核 — 拒絕 | 已拒絕申請，系統已通知消費者 | success |
| 快速審核 — 核准退貨 | 已核准退貨申請，系統通知信已發送 | success |
| 快速審核 — 核准換貨 | 已核准換貨申請，系統通知信已發送 | success |

#### 退換貨詳情（rma-detail）

| 觸發操作 | Toast 文字 | 類型 |
| --- | --- | --- |
| 拒絕原因為空，點擊提交 | 請填寫拒絕原因，才能提交審核結果 | error |
| 拒絕申請成功 | 已拒絕申請，系統已通知消費者拒絕原因 | success |
| 退貨地址為空，點擊儲存 | 請填寫退貨地址 | error |
| 儲存退貨地址成功 | 退貨地址已儲存，系統通知信已發送 | success |
| 確認已收到商品 | 已標記收到商品，請進行驗收 | success |
| 驗收通過 | 驗收完成，庫存已自動回補，退款處理中 | success |
| 驗收異常說明為空，點擊提交 | 請填寫異常說明，才能標記驗收異常 | error |
| 驗收異常成功 | 已標記驗收異常，系統已通知消費者 | success |
| 換貨確認庫存 | 換貨庫存已確認，請備貨並填寫出貨資訊 | success |
| 換貨出貨資訊儲存成功 | 換貨商品已出貨，消費者收到通知 | success |

#### 訂單設定（order-settings）

| 觸發操作 | Toast 文字 | 類型 |
| --- | --- | --- |
| 儲存設定成功 | 設定已儲存，新設定將套用於之後建立的訂單 | success |

#### 新增訂單（create-order）

| 觸發操作 | Toast 文字 | 類型 |
| --- | --- | --- |
| 訂單建立成功 | 訂單已建立！訂單編號：{訂單編號} | success |

> 注意：建立成功後 1.5 秒自動導航至訂單列表頁。
...
- **設計師這邊需要注意：**
  1. 訂單列表的「貨到付款未開發票」Tag 必須是紅色（`#F56C6C`）且加 icon，這是稅務合規的重要提示
...
- 訂單管理（獨立）：`html/訂單管理.html`
- 訂單管理（整合版）：`index.html` → 側邊選單 > 訂單管理（路由：`order-list`、`order-detail`、`rma-list`、`rma-detail`、`guest-query`、`guest-result`、`guest-rma`、`create-order`、`order-settings`）
