...
# Evomni — 會員前台個人中心 產品需求文件 (PRD) v1.2
...
消費者在商品頁點擊「收藏」圖示 → 商品加入心願清單 → 進入個人中心「心願清單」查看所有收藏 → 可直接加入購物車或移除。
...
- 商品有多種規格（顏色/尺寸等）：點擊「加入購物車」先展開規格選擇浮層（`<el-drawer>` 或 Bottom Sheet），選好後再加入
...
CREATE TABLE wishlists (
  ...
  variant_id      BIGINT UNSIGNED NULL,              -- 若商品有規格時指定 SKU
