...
| 商品管理 | 商品建立 | 新增單一商品完整資料 | 設定商品名稱、描述（富文本）、主圖/多圖（最多10張）、價格、特價、成本價、SEO 標題/描述/關鍵字、商品狀態、上下架檔期、溫層設定、重量設定、0元商品開關 | 商家管理員 |
...
**頁面佈局：**
- 頁首：麵包屑（商品中心 > 商品管理） + 右上角「+ 新增商品」主要操作按鈕（`#303133`，`!rounded-none`）
...
| 操作欄 | 「編輯」文字按鈕（藍色）、「複製」icon 按鈕（Tooltip: 複製此商品）、「刪除」icon 按鈕（紅色，二次確認）|
...
- 刪除確認 Modal：「確定要刪除「{商品名稱}」嗎？此操作無法復原。商品將進入回收狀態，已成立的訂單不受影響。」確認按鈕為 `danger` 色
- 批次刪除時若有商品有進行中訂單，顯示警告：「{N} 件商品有進行中的訂單，無法刪除，其餘商品已刪除」
...
| 純文字描述 | 商品詳細說明 | `<el-input type="textarea">` Optional；不含格式工具列；Placeholder：「請輸入商品詳細說明…」；不限字數 |
...
| 區塊 | 元件 | 規格 |
| --- | --- | --- |
| 商品狀態 | 狀態切換 | `<el-select>`；選項：已上架 / 已下架 / 草稿；預設「草稿」（新商品）|
...
- 商品軟刪除（`deleted_at` 時間戳），不做物理刪除；已關聯訂單的商品不允許永久刪除
...
CREATE TABLE products (
    ...
    is_free         TINYINT(1) DEFAULT 0,       -- 0元商品開關
