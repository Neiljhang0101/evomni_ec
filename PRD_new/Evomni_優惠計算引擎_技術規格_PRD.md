...
- 退款單下方固定揭露文案：「因加購品退回，原訂單滿額條件 NT$XXX 已未達成，差額由商家承擔」
...
### 8.2 daily snapshot 表的會計期間欄位 [設計取向待定]

**目前狀態**：本題影響 `reward_receivable_daily_snapshot` 表結構。

**取向 A — 不提供會計期間欄位**
- 系統行為：商家自行用日期區間查詢
- 工時：小
- 影響：商家若會計年度非曆年制（例：4 月起算），需自行計算

**取向 B — 提供 `fiscal_period` 欄位**
- 系統行為：商家後台可設定「會計年度起始月份」，引擎自動標記每筆 snapshot 的會計期間
- 工時：中
- 影響：商家可直接以會計期間切資料；商家後台需新增「會計年度設定」UI

**待定案題目**：
1. 商家對會計期間的彈性需求是否值得這個欄位？
2. 若做，「會計年度起始月份」設定是否影響其他模組？

**定案時機**：開發前與工程師討論確認。

---

### 8.3 退款單揭露文案可編輯性 [設計取向待定]

**目前狀態**：v1.2 已決議用系統內建固定文案，但是否要預留可編輯欄位的 schema 空間？

**取向 A — 系統內建固定文案**
- 系統行為：寫死於前端 component；變更需發版
- 工時：小
- 影響：v1.3 若改可編輯，要做資料遷移

**取向 B — 文案儲存於資料表，預留可編輯空間**
- 系統行為：新增 `system_settings.refund_disclosure_text` 欄位，v1.2 預設值為固定文案，後台暫不開放編輯
- 工時：略增（約 1 人日）
- 影響：v1.3 開放編輯時無需資料遷移

**待定案題目**：
1. v1.2 是否預留欄位避免 v1.3 遷移成本？
