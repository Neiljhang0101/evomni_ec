...
| 日期 | 2026/05/22 |
| 作者 | Una |
| 文件狀態 | **v4.5** — A1 LINE OA 定案（主要推播管道確定，移除所有可行性待確認標記）；Part4 §6.4 規格全數生效；Master PRD §4.2 / §6.2 / §7 #10 同步關閉 |
...
>
> **最後更新：2026/05/22（v4.5）— A1 LINE OA 定案（主要推播管道確定，§4.2 / §6.2 / §7 #10 全部關閉）；架構阻斷議題 A1 / A2 / A3 全數定案，工程師可進入結帳 + 行銷旅程模組開發**
...
| Part 3 | 訂單管理 | `Evomni_Part3_訂單管理_PRD.md` | v1.2 | ✅ **v1.2**（2026/05/21）— §6.2.A 新增訂單狀態 Tag R8 Tooltip 三層規格（8 種狀態 × What / Impact / Next Step）；v1.1：連動 Part4 v1.2 加購分區 + 退款邊界 + 雙模式追回 | ✅ 完成 |
| Part 4 | 行銷活動 | `Evomni_Part4_行銷活動_PRD.md` | v1.3 | ✅ **v1.3**（2026/05/21）— 新增進階電商包功能 plan-lock UI 規格（§6.2A Tab 鎖定 / §6.3 自動化旅程整頁 Banner + Sidebar 鎖定 / §6.4 LINE OA 整頁 Banner + 欄位 disabled）；v1.2：依會議決議大幅補強 6 章節 | ✅ 完成 |
...
| Part 6 | 會員管理 | `Evomni_Part6_會員管理_PRD.md` | v1.8 | ✅ **v1.8**（2026/05/22）— §6.12 通知信第 10 項「帳號系統自動停用通知」新增（連動前台登入 PRD §5.4）；進階電商包通知順移 #11–#14；v1.7：§6.12 通知信管理（13 種）/ §6.13 會員設定（全局三開關）；v1.6：§6.2.B 等級 Tag R8 Tooltip | ✅ 完成 |
...
| 會員前台個人中心 | `Evomni_會員前台個人中心_PRD.md` | v1.2 | ✅ **v1.2 連動 Part4 v1.2**（2026/05/21）— §6.5 點數/購物金頁新增「待入帳」卡片 + 明定會員端不顯示應收／註銷資訊 + Mode B 負餘額待擴充 UI 預留說明；v1.1：消費者前台不暴露方案名稱、心願清單旗標控制 | ✅ 完成 |
| CMS 選單 SEO 保護機制 | `Evomni_CMS選單SEO保護與路由邏輯_PRD.md` | v1.0 | ✅ **首次完整規格**（2026/05/04）— 補強 WTO 選單管理的 SEO 保護層：頁面未加入選單 Warning callout、移除節點前 301 redirect 設定彈窗、`page_slug_history` + `redirects` DB Schema、前台 routing 比對機制；同時正式文件化 CMS 頁面與電商商品的路由邏輯差異對照 | ✅ 完成 |
...
| 飛信電子報串接 | `Evomni_飛信電子報串接_PRD.md` | v1.1 | ✅ **v1.1 連動 Part4 v1.2**（2026/05/21）— §6.5 新增「應收餘額註銷前 30 天挽回通知」會員旅程（Email + LINE 完整範本、後台可調設定、邊界情境）；v1.0：Token 串接、一鍵匯出名單、飛信群組自動命名 | ✅ 完成 |
...
| 顧客與會員整合管理 | `Evomni_顧客與會員整合管理_PRD.md` | v1.3 | ✅ **v1.3**（2026/05/22）— §6.1 列表篩選新增「系統自動停用」選項（區分手動 / 系統觸發，連動前台登入 PRD §5.4）；v1.2：CM-1 / CM-2 定案；v1.1：補齊 §6.3（訪客轉會員設定 / 預設標籤）；v1.0：整合顧客列表 / Email 錨點合併 / 身分類型區分 / 邀請入會流程 | ✅ 完成 |
...
| **全域設定：電商設定** | `Evomni_全域設定_電商設定_PRD.md` | v1.1 | 補齊 §4.3 Row 1「全域設定規格待補」缺口；電商基本設定（幣別 7 種 / 訂單編號格式 / 發票公司抬頭 + 統一編號）、電商進階設定（購物車逾時 / 庫存警示門檻）；GS-1～4 四項 PM 決議均已定案：多幣別 Phase 2 規劃 / 前綴首次儲存鎖定 / 不加 Sidebar 紅點 / 發票公司抬頭獨立欄位 | ✅ **首次完整規格**（2026/05/22）|
| **購物車與結帳流程** | `Evomni_購物車與結帳流程_PRD.md` | v1.1 | **A2 + A3 定案**；v1.1：CC-1～CC-5 全數定案（訪客結帳後完整流程 / 購物車過期設定 / 一頁式商店共用結帳 / 0 元商品強制登入 / LP 優惠套用規則含商業情境）；v1.0：購物車機制（訪客 localStorage 7天 / 會員 DB 30天 / 合併規則）、結帳七區塊規格、庫存鎖定時機（訂單建立時 + Race Condition 保護）、逾時取消機制（信用卡 30 分/ATM 72 小時/貨到付款 NULL）、訂單建立觸發；A3 §2：進行中訂單定義（最終態排除法）、完整訂單主狀態、部分退款為事件記錄 | ✅ **v1.1 全定案**（2026/05/22）|
...
| 10 | ~~LINE Open API 串接可行性與成本評估~~ | ~~Part 4、Part 6~~ | Una | ✅ **已關閉（2026/05/22）** — PM 定案：LINE OA 為進階電商包主要推播管道，方向確定；技術實作細節（API 授權申請、成本）由工程師評估；Part 4 §6.4 完整規格全數生效，移除所有「若不可行」備援條件句；Email 為 fallback（無 LINE UID 時自動降級），已於各旅程規格中體現 |
| 11 | ~~美安串接條件與費用~~ | ~~第三方整合~~ | 廖紫茵 | ✅ **已關閉** — 美安串接定案為「可加費串接」（兩方案均支援），完整規格詳見 `Evomni_美安串接_PRD.md` v1.0 |
| 12 | 電商商品詳情頁是否提供「延伸閱讀：查看品牌故事」連結，指向形象站對應產品頁？ | 電商商品前台頁、形象站 CMS 產品 | — | 🟡 **建議方案**：若 `products.cms_product_id` 有值則顯示連結，反之不顯示；需老闆確認是否要這條連結 |
| 13 | 「形象站 CMS 產品」的相關產品推薦，是否允許跨系統推薦電商商品？ | CMS 相關產品功能 | — | 🟡 **建議**：不跨系統，各自維護相關產品，避免跨 DB 查詢複雜度；待確認是否接受此限制 |
...
| 17 | CMS 產品分類 與 電商商品分類 是否共用同一套分類設定？ | Part 2 商品中心、CMS 分類管理 | — | 🔴 **建議**：分開維護（CMS 分類偏品牌故事分類；電商分類需支援三層、對應廣告 Feed 分類）；需老闆確認接受各自獨立維護 |
| 18 | 美安串接 PRD 中提及「最高權限管理員」帳號可查看美安 Session 記錄與拋轉 Log——此「最高權限」是指 DB flag（`is_superadmin`）還是現有後台角色（Role-Based），尚無文件定義？ | 美安串接、後台角色控制 | 張皓評 | 🔴 **待定義** — `Evomni_美安串接_PRD.md` 提及權限控制但未規格化具體機制；需確認是另加 superadmin flag 還是整合進現有 RBAC；確認前工程師不可實作美安後台權限頁 |
