...
| 版本 | v1.3 |
| 文件狀態 | v1.2 — CM-1 / CM-2 兩項待確認事項關閉定案；預設標籤多語系後續規劃說明補入 |
...
```sql
-- contacts 表補充欄位（已存在於 CMS，補充電商整合欄位）
ALTER TABLE contacts
  ...
  ADD COLUMN invite_sent_at     TIMESTAMP       NULL    COMMENT '最近一次邀請加入會員信的寄送時間（7天冷卻期判斷用）',
...
CREATE TABLE customer_settings (
  ...
  auto_link_email            BOOLEAN           NOT NULL DEFAULT TRUE  COMMENT '自動連結相同 Email 帳號開關',
  invite_tag_id              BIGINT UNSIGNED   NULL                   COMMENT '邀請後自動加標籤 ID；NULL 表示不加',
  new_contact_tag_ids        JSON              NULL                   COMMENT '新顧客預設標籤 ID 陣列',
  new_member_tag_ids         JSON              NULL                   COMMENT '新會員預設標籤 ID 陣列',
  list_columns               JSON              NULL                   COMMENT '顧客列表顯示欄位設定',
  list_sort_column           VARCHAR(50)       NOT NULL DEFAULT 'created_at',
  list_sort_direction        ENUM('asc','desc') NOT NULL DEFAULT 'desc',
  updated_at                 TIMESTAMP         NULL ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id)
) COMMENT='顧客管理設定（單一 Row，id 固定為 1）';
```
