...
| 需求來源 | 2026/05/04 Una 補充指示：前台會員登入後個人頁面完整規格 |
| 文件狀態 | v1.0 首次完整規格 |
...
| 我的訂單 | 訂單列表 | 快速瀏覽所有訂單 | 依狀態分 Tab（全部/待付款/待出貨/已出貨/已完成/已取消/退換貨申請中）；每筆訂單卡片顯示產品縮圖、金額、狀態、操作按鈕 | 消費者 |
| 我的訂單 | 訂單詳情 | 查看單一訂單完整資訊 | 狀態時間軸、產品明細、收件人、付款方式、物流資訊、金額計算、退換貨歷程 | 消費者 |
...
| 心願清單 | 產品收藏 | 追蹤有興趣產品 | 收藏產品列表；可直接加入購物車；產品下架時顯示提示；此功能是否顯示由商家後台設定決定 | 消費者 |
...
| US-06 | 消費者 | 身為消費者，我想要收藏有興趣的產品，並在心願清單頁一鍵加入購物車，以便不用重新搜尋產品。 |
...
[搜尋列]
 [🔍 搜尋訂單編號或產品名稱 _______________]

[訂單卡片列表]
  ─────────────────────────────────────────────
  訂單編號：#EC-20260504-0001   下單日期：2026/05/04
                                [狀態 Tag：待出貨]
  [產品縮圖 60x60] 產品名稱（超過一行省略）     NT$ 1,280
  [產品縮圖 60x60] 產品名稱                    共 2 件
  （超過 3 件：顯示 +N 件）
                          [查看訂單] [申請退換貨] 等操作按鈕
  ─────────────────────────────────────────────
```
...
- 點擊「確認收貨」：`<el-message-box confirm>` 文字：「確認您已收到此訂單的產品？確認後訂單狀態將更新為完成。」
- 點擊「取消訂單」（待付款狀態）：彈窗說明「訂單取消後不可恢復，確定要取消嗎？」；取消成功後 Toast「訂單已取消」，Tab 自動切換至「已取消」
- 點擊「再次購買」：將訂單產品全部加入購物車（若產品已下架或刪除則跳過，並提示「部分產品已下架，無法加入購物車」）
...
| 使用限制 Tooltip | `<el-tooltip>` 完整文案，如：「此優惠券不適用於已折扣產品、加購產品及禮物卡。」 |
| 立即使用按鈕 | `<el-button type="primary" class="!rounded-none" size="small">`；點擊導向產品列表頁（若有指定分類則帶分類篩選）|
...
- 「立即使用」點擊後：進入產品頁，優惠券不會自動套用（需在結帳頁手動選擇），這是設計決策——不強迫套用，讓消費者自行決定
...
消費者在產品頁點擊「收藏」圖示 → 產品加入心願清單 → 進入個人中心「心願清單」查看所有收藏 → 可直接加入購物車或移除。
...
[產品格狀列表 Grid 3 欄（桌面）/ 2 欄（平板）/ 1 欄（手機）]
  ┌────────────────────┐  ┌────────────────────┐
  │  [產品圖 1:1]       │  │  [產品圖 1:1]       │
  │  產品名稱           │  │  ❌ 此產品已下架      │
  │  NT$ 1,280         │  │  產品名稱           │
...
| 元素 | 規格 |
| --- | --- |
| 產品圖片 | `aspect-ratio: 1/1`，`object-fit: cover`；下架產品加半透明遮罩 `rgba(0,0,0,0.4)` + 「已下架」文字 |
| 產品名稱 | 最多 2 行，超過省略 |
...
- 加入購物車失敗（已售完）：Toast Warning「此產品目前已售完，無法加入購物車」
- 產品有多種規格（顏色/尺寸等）：點擊「加入購物車」先展開規格選擇浮層（`<el-drawer>` 或 Bottom Sheet），選好後再加入

#### D. 防呆機制與錯誤預防

- 商家刪除產品後，心願清單中顯示「此產品已下架」，不自動從清單移除（讓消費者自行決定是否刪除，保留歷史收藏紀錄）
...
CREATE TABLE wishlists (
  ...
  variant_id      BIGINT UNSIGNED NULL,              -- 若產品有規格時指定 SKU
  created_at      TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY uq_member_product (member_id, product_id, variant_id),
...
CREATE TABLE member_sessions (
  id              BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  member_id       BIGINT UNSIGNED NOT NULL,
  token_hash      CHAR(64) NOT NULL,                  -- SHA-256 of JWT Token
  device_info     VARCHAR(255) NULL,                  -- User-Agent 解析結果，如「Chrome on macOS」
  ip_address      VARCHAR(45) NULL,                   -- IPv4 or IPv6
  last_active_at  TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  created_at      TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  is_revoked      TINYINT(1) NOT NULL DEFAULT 0,      -- 1 = 已登出/黑名單
...
-- 帳號刪除申請（新增）
CREATE TABLE member_delete_requests (
  id              BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  member_id       BIGINT UNSIGNED NOT NULL,
  requested_at    TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  confirm_token   CHAR(36) NOT NULL,                  -- UUID v4，Email 確認連結使用
  confirmed_at    TIMESTAMP NULL,                     -- Email 確認時間
  scheduled_delete_at TIMESTAMP NULL,                 -- 確認後 + 30 天
  cancelled_at    TIMESTAMP NULL,                     -- 冷靜期內重新登入取消
  status          ENUM('pending_confirm','scheduled','cancelled','completed') NOT NULL DEFAULT 'pending_confirm',
  UNIQUE KEY uq_member_active (member_id, status),
  FOREIGN KEY (member_id) REFERENCES members(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- member_addresses（已存於 Part 6，確認欄位齊全）
-- 若尚未建立，建議結構如下：
