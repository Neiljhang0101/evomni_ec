...
| v2.0 | 2026/05/04 | 全面改寫：模組更名為「數據中心」；新增同期群分析、購物車漏斗深度展開、廣告 ROI、產品加購漏斗、時段熱力圖、訂單來源分析、產品搜尋分析；補齊啟航 vs 進階功能分層；加入 site_mode 動態分頁邏輯與升級引導；完整 OLAP 架構說明與 DB Schema | Claude（依廖紫茵授權產出）|
| v1.0 | 2026/04/29 | 初稿：銷售概覽 / 產品分析 / 會員分析（含 RFM 5 分群）/ 行銷成效 四頁完整 UI/UX 規格 | Claude（依廖紫茵授權產出）|

# Evomni - 數據中心 產品需求文件 (PRD) v2.0
...
| 版本 | v2.0 |
| 日期 | 2026/05/04 |
| 需求來源 | Master PRD Part 5 + 後台左側導覽架構規劃 v1.1 + 舊版 PRD v1.1（2025/12/05）+ 市場電商分析需求調研 |
| 文件狀態 | ✅ **v2.0 全面改寫** — 電商分析四頁面完整 UI/UX 規格；補齊現代電商常用指標；啟航 vs 進階分層明確 |
...
> **📌 數據中心架構說明：** 數據中心是 Evomni 後台的統一分析入口。CMS 形象站的流量分析、SEO 報告、表單成效為 CMS 原生功能，本 PRD **不涵蓋** CMS 原生分頁（由 CMS 團隊維護）。本 PRD **僅涵蓋** 電商模組新增的四個分頁：銷售概覽、產品分析、會員分析、行銷成效。
...
**核心問題：**
商家每天操作訂單、上架產品、設定促銷，但不知道整體數字——哪些產品在賺錢、哪些顧客值得花力氣維繫、哪個促銷根本沒人用、購物車在哪一步流失最多人。沒有數據，所有決策靠直覺。
...
| 依賴模組 | 依賴資料 | 說明 |
| --- | --- | --- |
| 訂單管理（Part 3）| 訂單金額、狀態、時間、產品、UTM | 銷售概覽與行銷成效的核心資料來源 |
| 產品中心（Part 2）| 產品名稱、分類、庫存、PV/UV、加購紀錄 | 產品分析頁的品項與漏斗資料 |
...
| **產品分析** | 熱賣 Top 20 排行 | 快速找到明星產品 | 按銷售量或銷售金額排行前 20，附產品縮圖、庫存剩餘量、毛利率（有成本價時顯示）| 啟航＋進階 | 商家管理員 |
| 產品分析 | 滯銷產品提醒 | 早發現早清倉 | 列出設定期間銷量低於門檻的產品；附快速連結前往產品編輯頁 | 啟航＋進階 | 商家管理員 |
| 產品分析 | 品類銷售佔比 | 看哪個分類最賺錢 | 長條圖顯示各產品分類的銷售金額佔比與成長率 | 啟航＋進階 | 商家管理員 |
| 產品分析 | 產品加購漏斗 | 找出轉換效率差的產品 | 產品頁 PV → 加入購物車 → 完成購買，三段漏斗；標出「看的人多但買的少」的產品，提示可能需要改圖或調整售價 | 進階 🔒 | 行銷人員 |
| 產品分析 | 搜尋關鍵字分析 | 知道顧客在找什麼 | 前台站內搜尋的熱門關鍵字排行；標示「有結果」vs「無結果」比例；無結果的關鍵字提示商家考慮補充產品 | 進階 🔒 | 商家管理員、行銷人員 |
| 產品分析 | 庫存周轉分析 | 評估庫存健康程度 | 各產品的庫存周轉天數（平均庫存 ÷ 日均銷量）；標記過高庫存（積壓風險）與過低庫存（斷貨風險） | 進階 🔒 | 商家管理員 |
...
| **行銷成效** | 購物車轉換漏斗 | 找出流失最嚴重的環節 | 漏斗圖呈現：產品頁瀏覽 → 加入購物車 → 進入結帳 → 填寫資料 → 完成付款，五段各步驟人數與流失率；標示最大流失點 | 進階 🔒 | 行銷人員 |
...
**作為行銷人員，** 我想要看到哪些產品「瀏覽很多但轉換很差」的漏斗資料，以便於優先優化這些產品的主圖和描述，把流量變成訂單。
...
| 元件 | 規格 |
| --- | --- |
| 頁面 Tab | `<el-tabs>` 水平 Tab：銷售概覽 / 產品分析 / 會員分析 / 行銷成效；啟航方案的進階 Tab 顯示 `🔒` 後綴 + 點擊跳出升級彈窗 |
...
╚══════════════════════════════════════╝
```
使用 `<el-card>` + 疊加半透明遮罩（`background: rgba(255,255,255,0.85)`）；內部圖表呈現模糊預覽（`filter: blur(4px)`）讓商家知道有什麼但看不清楚，創造升級欲望。
...
| --- | --- | --- | --- |
| 總銷售金額（GMV）| SUM（已完成訂單金額）| `NT$ X,XXX,XXX` | 「已完成訂單的產品金額總和（不含取消/退款訂單）」|
...
進入「產品分析」Tab → 預設顯示熱賣 Top 20 + 品類佔比 → 可切換查看滯銷產品 / 加購漏斗（進階）/ 搜尋分析（進階）/ 庫存周轉（進階）。

#### B. 介面佈局與元件拆解（Figma Ready）

**佈局：** 熱賣排行表格（全寬，最重要）→ 品類銷售佔比（全寬）→ 滯銷產品提醒（全寬）→ 產品加購漏斗（進階）→ 搜尋關鍵字（進階）→ 庫存周轉（進階）
...
| --- | --- |
| 排名 | 1–20 數字 badge |
| 產品名稱 + 縮圖 | 點擊跳轉產品編輯頁（新分頁）|
| 銷售數量 | 選定時間範圍內已完成訂單的產品數量 |
| 銷售金額 | 銷售數量 × 售價（SUM）|
...
- 表格：產品名稱 / 庫存量 / 選定期間銷量 / 最後一筆訂單日期 / 「前往產品頁」快速連結
- 右上角「批次加入促銷」Button：勾選後可快速跳轉行銷活動建立頁，預填產品清單
...
完成購買         XX%   X,XXX 人
```
產品維度篩選：可選特定產品查看其個別漏斗（預設顯示全站聚合數據）。
...
| 搜尋次數 | COUNT |
| 有搜尋結果 | 比例（有產品命中）|
| 無搜尋結果 | 比例（0 件產品命中）；紅色標示提醒 |
| 建議 | 無結果關鍵字附「考慮新增此關鍵字的產品，或在相關產品的 SEO 設定中加入此關鍵字」|
...
| 🚀 高潛力顧客 | R 高 + F 中 + M 中 | 發送升等提醒 / 推薦高單價產品 | 藍色 Tag |
...
| 階段 | 說明 | 數據來源 |
| --- | --- | --- |
| 產品頁瀏覽 | 訪問至少一個產品詳情頁的 UV | `product_stats.uv` |
...
| 形象站 + 電商（Hybrid）| 流量分析、SEO 報告、表單成效（CMS）＋ 銷售概覽、產品分析、會員分析、行銷成效（電商）|
| 純電商站 | 銷售概覽、產品分析、會員分析、行銷成效（電商）|
...
> - 標題：「解鎖電商數據分析」
> - 內容：「開通電商方案後，即可查看銷售趨勢、產品轉換率、顧客 RFM 分群等電商專屬分析報表。」
...
- 這次規格是「數據中心」模組（電商部分），包含四個分頁：銷售概覽、產品分析、會員分析、行銷成效；CMS 原生的流量分析/SEO報告/表單成效不在本 PRD 範圍內
...
- 這個模組依賴：訂單管理（Part 3）、產品中心（Part 2）、會員管理（Part 6）、行銷活動（Part 4）、發信系統、廣告投放工具，需等以上模組的資料結構穩定後才能開發
