...
| 半人工廠商設定 | 倉儲設定與品項對照 | 設定 BOXFUL / ECFIT 倉庫代碼與產品 SKU 對照 | 表格型設定：本系統 SKU ↔ 倉儲廠商 SKU 對照；支援 CSV 批次匯入 | 商家管理員 |
...
**US-06**　身為商家管理員，我想要設定 BOXFUL 倉儲時能上傳 SKU 對照 CSV，以便於批次對應本系統產品與倉庫 SKU，不需要逐一手動填入。
...
- 頁籤：`待上傳追蹤號 (N)` / `已完成`
- 表格欄位：訂單號 / 消費者姓名 / 產品名稱 / 付款時間 / 追蹤號（可編輯欄位）/ 操作
...
- 說明文字：`請將本系統的產品 SKU 對應至倉儲廠商的 SKU，以便下單時系統正確通知倉庫揀貨。`
- 表格欄位：本系統 SKU / 產品名稱 / 倉儲廠商 SKU / 最後更新時間 / 操作（編輯/刪除）
- 操作按鈕：
  - `+ 新增對照`（逐筆新增）
  - `批次匯入`（CSV 格式，支援下載模板，格式：`系統SKU,倉儲SKU`）
  - `下載對照表`（匯出目前設定為 CSV）
- 未對照的 SKU：表格頂部顯示 Warning Banner：`有 [N] 個產品 SKU 尚未對照，可能導致訂單出貨異常。` + 「前往設定」按鈕
...
- 若新增對照時輸入的「本系統 SKU」不存在於產品資料，欄位即時紅框並提示：`找不到此 SKU，請確認產品中心是否已建立此產品`
