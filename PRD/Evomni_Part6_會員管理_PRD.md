...
| 版本 | 日期 | 修改內容 | 修改人 |
|------|------|----------|--------|
| v1.6 | 2026/05/05 | §6.11 新增三項設定：啟用會員功能主開關（關閉後前台移除登入/註冊入口）、驗證機制（電子郵件驗證/人工驗證二選一）、前台會員可編輯欄位（Checkbox 控制）；DB Schema 補充 3 個新 shop_settings key；同步更新 UX Spec Screen 8 | Claude（依廖紫茵授權產出）|
| v1.5 | 2026/05/05 | 新增 §6.11 後台會員設定（自動成為會員/生日雙倍點數/推薦功能開關）；新增 §6.12 通知信管理（9 種 NT Email 的開關與模板編輯規格）；更新 §2.2 功能總覽表新增兩行；同步更新後台左側導覽架構：會員中心新增「設定」「通知信管理」兩個 nav 項目 | Claude（依廖紫茵授權產出）|
...
| 版本 | v1.5 |
| 日期 | 2026/05/05 |
| 需求來源 | Master PRD v1.0 Chapter 5（P0）、方案規格 V1.1、PRD V3 §3.2.8 |
| 文件狀態 | **v1.5 更新** — 新增 §6.11 後台會員設定、§6.12 通知信管理；更新 §2.2 功能總覽 |
| 作者 | Claude（依廖紫茵授權產出） |
...
| 貨到通知 | 缺貨產品訂閱 | 兩方案共有 | 挽回潛在失單 | 產品售完時前台顯示「到貨通知」按鈕；消費者點擊登記；補貨時 NT 批量通知 | 消費者 |
| 產品追蹤清單 | 心願清單 | 進階電商包 | 追蹤感興趣產品 | 消費者可收藏產品至追蹤清單；商家後台可查看哪些產品被收藏最多次 | 消費者、商家管理員 |
| 會員黑名單 | 黑名單管理 | 進階電商包 | 阻止惡意消費者 | 商家可將會員加入黑名單，黑名單會員無法下單；支援加入原因記錄與解除黑名單 | 商家管理員 |
| 後台會員設定 | 行為開關設定 | 兩方案共有 | 集中管理會員模組全局開關 | 商家可在此頁開啟/關閉：自動成為會員（首購建帳號）、推薦功能；進階方案另可開關生日雙倍點數 | 商家管理員 |
| 通知信管理 | 會員通知 Email 設定 | 兩方案共有（部分進階方案限定）| 管理所有會員相關自動發信 | 後台可開關各類通知信（歡迎/升等/降等/點數到期/生日/推薦等）並自訂郵件主旨與內文模板；提供預覽與測試發送 | 商家管理員 |
...
| US-07 | 消費者 | 身為心願清單使用者，我想要將喜歡的產品加入追蹤清單，以便我之後購買時不用重新搜尋。 |
...
**訂單記錄 Tab：**
- `<el-table>`；欄位：訂單編號（可點擊）、日期、產品摘要、金額、狀態
- 支援分頁，每頁 10 筆
...
- 計算公式：`FLOOR(訂單產品實付金額 ÷ 消費比率) 點`（不含運費、不含點數折抵部分）

**推薦機制：**
- 推薦人 A 分享推薦連結（含 `ref=A的會員ID` 參數）
...
| 品類愛好者 | 同一產品分類完成訂單次數 ≥ 3 次；自動偵測購買頻次最高的分類，並記錄分類名稱 | `#409EFF`（藍） | Part 4 優惠券發放可篩選此標籤 |
...
每個條件行由 3 個元件組成：
1. 篩選欄位 `<el-select>`：累計消費金額 / 購買次數 / 最後購買日距今天數 / 會員等級 / 會員標籤（含系統標籤+自定義標籤）/ 產品分類購買記錄 / 加入日期 / 身分類型（顧客/會員）
...
**後台路徑：** 「產品管理」→ 產品詳情 →「到貨通知訂閱者」Tab（或在「會員管理」→「貨到通知」）
...
```sql
-- 在 shop_settings 或 global_settings 表新增以下設定項目
-- （若現有全域設定採用 key-value 模式，則新增對應 key）

INSERT INTO shop_settings (key, value, type) VALUES
  ('member_verify_method', 'email', 'string'),          -- 'email' | 'sms'
  ('member_verify_email_expire_hours', '24', 'integer'), -- 12 | 24 | 48 | 72
  ('member_verify_email_resend_cooldown', '60', 'integer'), -- seconds
  ('member_verify_sms_provider', 'mitake', 'string'),   -- 'mitake' | 'every8d' | 'custom'
  ('member_verify_sms_api_account', '', 'string'),
  ('member_verify_sms_api_key', '', 'encrypted'),       -- ⚠️ 加密儲存，不可明文
  ('member_verify_sms_custom_url', '', 'string'),
  ('member_verify_sms_otp_expire_minutes', '5', 'integer'),
  ('member_verify_sms_resend_cooldown', '60', 'integer');

-- API 金鑰加密：建議使用 Laravel 的 Crypt::encryptString() 儲存，讀取時解密
```
...
> 此頁集中管理會員模組的全局行為開關，讓商家無需進入各子功能就能快速控制核心行為。設定儲存後立即生效。

#### A. 頁面結構

```
[頁面標題：會員設定]
說明文字：「管理會員模組的全局行為設定。所有設定修改後立即生效。」

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【啟用會員功能】⚠️ 主開關，位於頁面最頂部

  啟用會員功能
  ┌─────────────────────────────────────────────────┐
  │ 關閉後，電商前台將移除登入與會員註冊入口，     │
  │ 所有需要登入才能使用的功能（訂單追蹤、點數    │
  │ 查詢、個人中心）將全數停用。                   │
  │ 現有會員帳號資料不會被刪除，重新開啟後可繼續   │
  │ 使用。                                          │
  │                                                 [開啟 ●] │
  └─────────────────────────────────────────────────┘
  關閉時彈出確認 Dialog（見 §C 儲存行為）

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【驗證機制】

  說明：選擇消費者在前台申請會員時的驗證方式。切換後僅影響新申請，已啟用的既有會員不受影響。

  ● 電子郵件驗證（預設，推薦）
      系統自動寄出驗證信，消費者點擊信中連結後帳號立即啟用。無額外費用。
      ▶ 進階設定請至「全域設定 → 電商進階設定 → 會員驗證設定（§6.10）」

  ○ 人工驗證
      消費者填寫申請表後，帳號進入「待審核」狀態，由後台管理員手動核准或拒絕。
      核准後，系統自動發送歡迎信並啟用帳號。
      ⚠️ 開啟人工驗證後，後台會員列表將新增「待審核」狀態篩選與快速審核操作。
...
【帳號建立設定】
  ...
  自動成為會員
  ┌─────────────────────────────────────────────────┐
  │ 消費者完成首次購買後，系統自動以購買 Email     │
  │ 建立會員帳號，並以 NT 發送帳號開通通知。       │
  │                                                  │
  │ 開啟後：訪客首購 → 自動建帳號 → NT 發送密碼   │
  │ 關閉後：訪客需自行至前台手動註冊               │
  │                                                 [開啟 ●] │
  └─────────────────────────────────────────────────┘
...
【前台會員可編輯欄位】
  ...
  說明：勾選的欄位，會員可在前台「個人中心 → 基本資料」頁自行修改。
        取消勾選後，欄位在前台顯示為唯讀。
  ...
  ⚠️ Email 為帳號識別錨點，永遠不可由會員自行修改（固定唯讀，不受此設定影響）。
  ⚠️ 密碼修改入口不受此設定控制，會員隨時可自行修改密碼。
...
【推薦功能設定】
  ...
  ┌─────────────────────────────────────────────────┐
  │ 開啟後，會員個人中心顯示「推薦連結」；          │
  │ 被推薦人首購完成後，雙方自動獲得點數獎勵。      │
  │ 推薦人/被推薦人獎勵點數請至「點數設定」調整。   │
  │                                                 [開啟 ●] │
  └─────────────────────────────────────────────────┘

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【進階設定】（進階電商包，啟航方案此區塊顯示鎖定 Banner）

  生日雙倍點數
  ┌─────────────────────────────────────────────────┐
  │ 開啟後，會員在其生日所在月份完成的訂單，         │
  │ 回饋點數自動 × 2。                               │
  │ 生日賀卡 Email 亦於生日月份第 1 天自動發送。     │
  │                                                 [關閉 ○] │
  └─────────────────────────────────────────────────┘
...
| 元件 | 規格 | 說明 |
| --- | --- | --- |
| 啟用會員功能 Switch | `<el-switch>`；置頂，卡片外框使用 `border: 2px solid #DCDFE6`（關閉時變 `border-color: #F56C6C`）| 關閉時觸發確認 Dialog |
| 驗證機制 Radio Group | `<el-radio-group>`；垂直排列；每個選項下方有灰色說明文字 | 切換後儲存按鈕高亮 |
| 「電子郵件驗證」進階設定連結 | 文字連結 color `#409EFF`；font-size 12px | 跳轉至 §6.10 設定頁 |
| 前台可編輯欄位 Checkbox Group | `<el-checkbox-group>`；垂直排列；預設 6 個勾選 | 切換後儲存按鈕高亮 |
| Email 唯讀說明 | `<el-alert type="warning" show-icon>` size="small" | 說明 Email 不受此設定影響 |
| 每個功能 Switch（帳號建立/推薦/進階）| `<el-switch>` | 開 = `#409EFF`，關 = `#DCDFE6`；Label 在 Switch 右側顯示「已開啟」/「已關閉」 |
| 功能說明文字 | `<p class="text-sm text-gray-500">` | 描述此開關影響的行為，避免商家誤操作 |
| 進階功能鎖定 Banner | `<el-alert type="info">` | 文字：「此功能需升級至進階電商包方可使用。」；附「了解升級方案」連結（導向方案管理頁） |
| 儲存按鈕 | `<el-button type="primary">` | 文字：「儲存設定」；任一設定有變更後才高亮，未變更時灰色（disabled 樣式，可點） |

#### C. 儲存行為

- 儲存成功 Toast：`「會員設定已更新，設定立即生效」`
- 儲存失敗 Toast：`「儲存設定失敗，請稍後再試。」`

**確認 Dialog 觸發規則：**

| 操作 | Dialog 內容 |
| --- | --- |
| 關閉「啟用會員功能」| 標題：`確認停用會員功能？` / 說明：`停用後，電商前台將移除登入與會員註冊入口。現有會員將無法登入前台，但帳號資料不會被刪除，重新開啟後可繼續使用。` / 按鈕：`[取消] [確認停用]`（紅色） |
| 關閉「自動成為會員」| 標題：`確認關閉自動成為會員？` / 說明：`關閉後，訪客首購將不再自動建立會員帳號。已建立的會員帳號不受影響。` / 按鈕：`[取消] [確認關閉]`（紅色） |
| 切換驗證機制至「人工驗證」| 標題：`切換為人工驗證？` / 說明：`切換後，所有新申請的會員帳號將進入「待審核」狀態，需由後台管理員手動審核。切換不影響已啟用的既有會員帳號。` / 按鈕：`[取消] [確認切換]` |

#### D. DB Schema

```sql
INSERT INTO shop_settings (key, value, type) VALUES
  ('member_module_enabled', '1', 'boolean'),        -- 啟用會員功能主開關；1=開, 0=關
  ('member_verification_method', 'email', 'string'), -- 驗證機制；'email' | 'manual'
  ('member_editable_fields', '["name","phone","birthday","gender","avatar","address"]', 'json'),
                                                     -- 前台會員可編輯欄位清單
  ('member_auto_register', '1', 'boolean'),          -- 1=開, 0=關
  ('member_referral_enabled', '1', 'boolean'),
  ('member_birthday_double_points', '0', 'boolean'); -- 預設關閉（進階方案功能）
```

**人工驗證相關 members 表狀態補充：**

| `members.status` 值 | 說明 |
| --- | --- |
| `pending` | 申請中，待人工審核（驗證機制＝人工驗證時） |
| `active` | 已啟用（Email 驗證完成 or 人工核准）|
| `rejected` | 申請被拒絕（人工驗證拒絕）|
| `suspended` | 帳號被商家停用 |
| `blacklisted` | 黑名單 |

> 📌 工程師注意：人工驗證開啟後，後台 Screen 1（會員列表）需新增「待審核」狀態篩選器，並在列表操作欄新增「核准」/「拒絕」快速操作（進入詳情頁亦可操作）。此為 §6.1 連動修改，請同步評估工作量。
...
**後台路徑：** 側邊欄「會員中心」→「通知信管理」
**路由：** `/members/notifications`

> 本頁管理由會員模組觸發的所有系統自動 Email。商家可針對每一種通知個別開關，並自訂郵件主旨與內文（支援變數替換）。實際發信由 NT（發信模組）執行，本頁僅管理模板與開關。

#### A. 通知類型總表

| # | 通知名稱 | 觸發條件 | 方案 | 預設狀態 |
| --- | --- | --- | --- | --- |
| 1 | 歡迎信（手動/第三方登入） | 消費者 Email 驗證完成或第三方登入首次 | 兩方案 | ✅ 開啟 |
| 2 | 帳號開通通知（自動建帳號） | 首購後自動建立帳號（§6.11 開關開啟時） | 兩方案 | ✅ 開啟 |
| 3 | 升等恭喜通知 | 消費者消費達升等門檻，自動升等後 | 進階電商包 | ✅ 開啟 |
| 4 | 保級預警通知 | 等級距到期日 ≤ 30 天且尚未達保級門檻 | 進階電商包 | ✅ 開啟 |
| 5 | 降等通知 | 等級到期後未達保級條件，系統自動降等 | 進階電商包 | ✅ 開啟 |
| 6 | 點數到期提醒 | 點數將於 X 天後到期（X 設定於「點數設定」頁） | 兩方案 | ✅ 開啟 |
| 7 | 生日賀卡 | 會員生日月份第 1 天（§6.11 生日雙倍點數開啟時） | 進階電商包 | ✅ 開啟 |
| 8 | 推薦獎勵通知（推薦人） | 被推薦人首購完成後，通知推薦人已獲得點數 | 兩方案 | ✅ 開啟 |
| 9 | 推薦獎勵通知（被推薦人） | 首購完成後，告知被推薦人已獲得首購獎勵點數 | 兩方案 | ✅ 開啟 |

#### B. 頁面佈局

```
[頁面標題：通知信管理]
[說明文字]：設定會員相關自動發信。關閉通知後，對應事件觸發時系統將不發送 Email。

[通知列表，每列一種通知]

─────────────────────────────────────────────────────────
 歡迎信（手動/第三方登入）        兩方案     [● 已開啟]  [編輯模板]
 消費者 Email 驗證完成或第三方首次登入後自動發送
─────────────────────────────────────────────────────────
 帳號開通通知（自動建帳號）        兩方案     [● 已開啟]  [編輯模板]
 需搭配「設定 > 自動成為會員」開啟
─────────────────────────────────────────────────────────
 升等恭喜通知                      進階方案   [● 已開啟]  [編輯模板]
 消費達升等門檻後自動發送，信中含新等級名稱與權益
─────────────────────────────────────────────────────────
 保級預警通知                      進階方案   [● 已開啟]  [編輯模板]
 等級距到期日 ≤ 30 天時發送，說明距保級門檻差距
─────────────────────────────────────────────────────────
 降等通知                          進階方案   [● 已開啟]  [編輯模板]
 等級到期未達保級條件後發送
─────────────────────────────────────────────────────────
 點數到期提醒                      兩方案     [● 已開啟]  [編輯模板]
 提前天數由「點數設定」頁的「到期提醒提前天數」控制
─────────────────────────────────────────────────────────
 生日賀卡                          進階方案   [○ 已關閉]  [編輯模板]
 需搭配「設定 > 生日雙倍點數」開啟；生日月份第 1 天發送
─────────────────────────────────────────────────────────
 推薦獎勵通知（推薦人）            兩方案     [● 已開啟]  [編輯模板]
 被推薦人首購完成後通知推薦人點數已到帳
─────────────────────────────────────────────────────────
 推薦獎勵通知（被推薦人）          兩方案     [● 已開啟]  [編輯模板]
 被推薦人首購完成後通知本人點數已到帳
─────────────────────────────────────────────────────────
```
...
點擊「編輯模板」後從右側滑出 Drawer（寬度 600px），包含：
...
```
郵件主旨
[__________________________________________]  （單行輸入；最多 100 字）
提示：可使用變數 {{member_name}}、{{shop_name}}

郵件內文
[富文字編輯器（基礎格式：粗體/斜體/連結/換行）]
（最多 3,000 字）
提示：可使用以下變數（點擊插入）：
  [{{member_name}}] [{{shop_name}}] [{{level_name}}] [{{points_amount}}]
  [{{points_expire_date}}] [{{referral_link}}] [{{reward_points}}]

  （每種通知類型的可用變數不同，僅顯示該通知支援的變數）
...
| 通知 | 專屬變數 |
| --- | --- |
| 帳號開通 | `{{temp_password}}` — 系統產生的初始密碼 |
| 升等 | `{{level_name}}` — 新等級名稱；`{{level_benefits}}` — 等級權益說明文字 |
| 保級預警 | `{{level_name}}`；`{{days_remaining}}` — 距到期天數；`{{gap_amount}}` — 距保級門檻差距 NT$ |
| 降等 | `{{old_level_name}}`；`{{new_level_name}}` |
| 點數到期 | `{{points_amount}}` — 即將到期點數；`{{points_expire_date}}` — 到期日 |
| 生日賀卡 | `{{birthday_month}}` — 生日月份 |
| 推薦獎勵 | `{{reward_points}}` — 獲得點數；`{{referee_name}}` — 被推薦人姓名（推薦人版）|

**預覽功能：**
- 點擊「預覽 Email」→ 在 Drawer 右側或下方顯示模擬渲染結果（以當前登入管理員資訊填入變數）
- 預覽不實際發信

**還原為預設模板：**
- 確認框：`「確定要還原為預設模板嗎？目前的自訂內容將會遺失。」` → 確認後還原

#### D. 元件規格

| 元件 | 規格 |
| --- | --- |
| 開關 | `<el-switch>`；與方案鎖定連動（啟航方案無法開啟進階方案通知） |
| 進階方案通知在啟航方案 | Switch 顯示灰色鎖定狀態；Hover 顯示 Tooltip：「此通知需升級至進階電商包」 |
| 通知列 | `<el-card>` 每列；包含名稱、說明、方案 Tag、Switch、「編輯模板」按鈕 |
| 模板編輯器 | 富文字編輯器（建議 Quill.js 或等效，禁止嵌入 `<script>` 標籤） |
| Drawer | `<el-drawer direction="rtl" size="600px">` |
| 儲存 Toast | `「[通知名稱] 模板已更新」` |
...
| 通知 | 相依設定 |
| --- | --- |
| 帳號開通通知 | 需搭配 §6.11「自動成為會員」開啟，關閉後此通知無觸發時機但仍可設定模板 |
| 生日賀卡 | 需搭配 §6.11「生日雙倍點數」開啟，關閉時此通知自動停發（即使 Switch 開啟） |
| 推薦獎勵通知（兩則） | 需搭配 §6.11「推薦計畫」開啟，關閉推薦計畫後兩則推薦通知自動停發 |
| 點數到期提醒 | 提前天數由 §6.4「點數設定」→「到期提醒提前天數」欄位決定，本頁無法修改天數 |

#### F. DB Schema

```sql
-- 通知開關與模板儲存
CREATE TABLE member_notification_templates (
  id            INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  shop_id       INT UNSIGNED NOT NULL,
  notification_type ENUM(
    'welcome',            -- 歡迎信
    'account_created',    -- 帳號開通（自動建帳號）
    'level_upgrade',      -- 升等恭喜
    'level_retention_warning', -- 保級預警
    'level_downgrade',    -- 降等
    'points_expiring',    -- 點數到期提醒
    'birthday',           -- 生日賀卡
    'referral_reward_referrer',  -- 推薦獎勵（推薦人）
    'referral_reward_referee'    -- 推薦獎勵（被推薦人）
  ) NOT NULL,
  is_enabled    TINYINT(1) DEFAULT 1,
  subject       VARCHAR(200) NOT NULL,
  body_html     MEDIUMTEXT NOT NULL,
  updated_at    DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uq_shop_type (shop_id, notification_type)
);
```
...
- 扣回公式：`total_spent -= 退款產品金額（不含可退運費）`
...
每日排程查詢：
```sql
WHERE level_expires_at BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 30 DAY)
  AND retention_warning_sent_at IS NULL
```
發送後更新 `retention_warning_sent_at = NOW()`。
...
CREATE TABLE point_batches (
  ...
  -- Enum: order_reward, manual_add, manual_deduct, referral_reward,
  --       referral_revoked, birthday_bonus, system_grant
...
點數扣除順序：先到期先扣（`ORDER BY expires_at ASC FOR UPDATE`）。
...
- **工程師這邊需要注意：**
  ...
  2. 黑名單的 `is_blacklisted` 欄位必須跳過 Redis 快取，直接讀 DB，確保即時生效
  3. 點數系統建議用獨立的 `point_batches` 表（每次發放建一筆記錄，含到期日），不要只存總餘額，以支援精確的到期計算
  4. 分眾篩選器的多條件 AND/OR 查詢要注意 SQL 查詢效能，建議對 `orders` 表的 `member_id`、`status`、`created_at` 建立複合索引
- **設計師這邊需要注意：**
