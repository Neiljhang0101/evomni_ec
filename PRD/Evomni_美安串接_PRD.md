...
**核心問題：**
美安平台的會員（IBO）會帶消費者到客戶網站消費，美安需要取得對應的訂單資料（包含：哪些訂單是從美安導入、訂單金額、產品資訊）以計算佣金。現有流程需要商家手動操作、手動匯出報表，容易出錯，且美安的驗測流程對產品資料格式有嚴格要求（XML 格式、JPG 圖片、必填欄位等），稍有不符就無法通過美安驗測。
...
| 依賴模組 | 用途 |
| --- | --- |
| Part 2 產品中心 | 產品資料來源（名稱、分類、定價、售價、圖片、說明）用於生成 XML Feed |
| Part 3 訂單管理 | 訂單建立事件觸發傳送；訂單詳情頁新增美安操作按鈕 |
| 媒體庫 | 產品圖片 URL 來源；美安限 JPG，系統需轉檔處理 |
...
| 產品 XML Feed | 自動生成美安格式 XML | 讓美安系統能取得產品目錄 | 系統依美安 XML 規範自動生成產品目錄 XML；每日定時更新；圖片自動轉換為 JPG（美安要求）；已停用分類的產品不出現在 XML 中 | Evomni 系統（定時執行）|
| 產品 XML Feed | 統一分類名稱設定 | 解決無分類產品的 XML 必填問題 | 若產品未設定分類，美安 XML 的 `<category>` 欄位將為空（驗測不通過）；後台提供「美安預設分類名稱」輸入框，無分類產品統一使用此名稱填入 `<category>` | 商家管理員 |
...
| # | 角色 | 故事 |
| --- | --- | --- |
| US-01 | 商家管理員 | 身為商家管理員，我想要系統自動生成符合美安格式的 XML，以便美安能定期抓取我的產品目錄，不需要我手動整理 XML 格式。 |
...
說明（小字）：「若剛剛修改了產品資料，可點此立即重新生成 XML，無需等待每日排程」
```
...
| 欄位 | 元件 | 驗證規則 | 說明 |
| --- | --- | --- | --- |
| 美安預設分類名稱 | `<el-input>` | 選填；最多 50 字；Placeholder：「例：精選產品」 | 說明文字：「若產品未設定分類，美安 XML 的 category 欄位將使用此名稱。若產品已設定分類，以實際分類名稱優先。」 |
...
⚠️ 注意：產品說明如果只有圖片或影片（無文字），美安 XML 驗測將不通過。
請確認每件產品至少有一段文字說明。
```
...
CREATE TABLE meian_settings (
  ...
  default_category VARCHAR(100) DEFAULT NULL COMMENT '無分類產品的預設分類名稱',
...
- **工程師這邊需要注意：**
  ...
  2. **XML 圖片必須是 JPG**，系統需在生成 XML 時自動將 PNG/WebP 產品主圖轉換為 JPG 格式另存，不影響前台顯示
...
- 這個模組依賴 **Part 2 產品中心**（XML Feed 產品資料）、**Part 3 訂單管理**（訂單狀態事件、詳情頁）
