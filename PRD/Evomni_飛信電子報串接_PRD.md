...
| 版本 | 日期 | 修改內容 | 修改人 |
|------|------|----------|--------|
| v1.0 | 2026/05/05 | 初稿建立：飛信電子報一鍵開通、Token 串接、名單同步、快速登入完整規格 | Claude（依廖紫茵授權產出）|

---

# Evomni — 飛信電子報串接 產品需求文件 (PRD)
...
| 版本 | v1.0 |
| 日期 | 2026/05/05 |
| 需求來源 | Webtech2 飛信電子報說明文件（吳毓祥 2024-09-05）；廖紫茵需求 |
| 文件狀態 | ✅ 初版完成 |
...
```sql
-- 飛信串接設定
CREATE TABLE flydove_settings (
  id          BIGINT AUTO_INCREMENT PRIMARY KEY,
  store_id    BIGINT NOT NULL,
  api_token   TEXT DEFAULT NULL COMMENT 'AES-256 加密',
  status      ENUM('connected','disconnected','token_invalid') DEFAULT 'disconnected',
...
-- 匯出紀錄
CREATE TABLE flydove_export_logs (
  id              BIGINT AUTO_INCREMENT PRIMARY KEY,
  store_id        BIGINT NOT NULL,
  total_count     INT DEFAULT 0 COMMENT '本次匯出總筆數',
  success_count   INT DEFAULT 0,
  failed_count    INT DEFAULT 0,
  failed_emails   JSON DEFAULT NULL COMMENT '失敗的 Email 陣列',
  flydove_group_name VARCHAR(100) DEFAULT NULL COMMENT '在飛信系統的群組名稱',
  status          ENUM('pending','processing','success','partial_failed','failed') DEFAULT 'pending',
  triggered_by    BIGINT DEFAULT NULL COMMENT '操作人 user_id',
  created_at      DATETIME DEFAULT CURRENT_TIMESTAMP,
