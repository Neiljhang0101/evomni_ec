...
| 依賴模組 | 用途 |
| --- | --- |
| Part 2 產品中心 | 產品資訊（ID、名稱、分類、價格）注入 ViewContent / AddToCart 事件 |
| Part 3 訂單管理 | 訂單完成事件注入 Purchase 事件（訂單金額、產品清單、運費）|
...
| 產品詳情頁瀏覽 | view_item | ViewContent | item_id, item_name, item_category, price, currency（TWD），content_type: 'product' |
...
- 這個模組依賴 **Part 2 產品中心**（產品參數）、**Part 3 訂單管理**（Purchase 事件）、**Part 6 會員管理**（CompleteRegistration 事件）
