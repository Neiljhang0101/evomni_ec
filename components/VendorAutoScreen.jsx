...
function VendorAutoScreen({ vendor, onBack }) {
  ...
  const [enabled, setEnabled] = React.useState(vendor.status === '已啟用' || vendor.status === '連線異常');
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <StatusBadge status={enabled ? '已啟用' : (vendor.status === '連線異常' ? '連線異常' : '已停用')} />
              </div>
            </div>
          </div>
